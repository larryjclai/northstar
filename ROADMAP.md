...
### ✅ 3.2 個股貢獻分析（Attribution）— 已完成（commit fc945991）
- `buildReturnAttribution`：各持倉貢獻 = 目前持股 × 期間價格變化（fixed-basket），加總精確等於期間市值變化。
- 投資分析「報酬貢獻」卡：排序條列 + bar（正綠負紅）+ 金額 + 佔比，前 6 + 其他、期間合計；無價標的揭示。
- 瀏覽器實測：合計 +34,465 = 期間市值變化 +34,465、佔比加總 100%。

### ✅ 3.3 股利與收益分析 — 已完成（commit 87b4cc8d）
- `buildDividendAnalysis`：cashDividend 依年度/個股彙總（淨額、依記錄日換算主幣）、TTM 股利、以目前市值計算殖利率。
- 投資分析「股利」區：近一年股利/殖利率/累計三指標 + 年度股利 bar + 個股股利貢獻 bar；無股利時整區收合。
- demo 補 4 筆現金股利（皆在持倉買入後）；瀏覽器實測 TTM 3,870、殖利率 2.38% 一致。
- **未做**：已實現損益年度報表（報稅用）留待後續。

### ✅ 3.4 幣別曝險 — 已完成（commit 73f41a79）
- `buildCurrencyExposure`：依計價幣別彙總持倉現值（換算主幣），佔比加總 100%，單一幣別不顯示。
- 投資分析「配置」區「幣別曝險」卡：各幣別 bar + 市值 + 佔比。
- demo 新增 Firstrade（USD）+ VOO 持倉與 USD 股價，多幣別可離線驗證；實測 TWD 65.6% / USD 34.4%。
- **v1 範圍**：僅投資持倉、不含現金帳戶（help 已說明）；「本月匯率變動對淨值貢獻」留待後續。

---

**Phase 3 全部完成**（3.1 TWR、3.2 個股貢獻、3.3 股利、3.4 幣別曝險、3.5 買賣標記）。投資分析分頁現有：報酬口徑三口徑、報酬貢獻、股利、幣別曝險、風險指標、配置漂移、Portfolio vs Benchmark。
