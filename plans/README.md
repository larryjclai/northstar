...
| 068 | ETF sector + country/region breakdown by holdings (instead of 「未知分類」) | P2 | T0 ~0.5-1d / T1 ~1.5-2.5d | T0 LOW / T1 MED | 067, 069 (both MERGED) | READY — Phase 0 spike DONE (doc 9db191a2). Verdict: Yahoo topHoldings blocked; no TWSE ETF-holdings feed; only Yahoo TW (TW ETFs). Operator signed off: **JSON-API-only (NO HTML scraping), weekly bg refresh**. Tier 0 (no network): direct-holding country map + ETF 「ETF/基金」 bucket + manual tag (069 lock) — build unconditionally. Tier 1 (Yahoo-TW JSON enrichment): gated on a 1h unattended-confirmation; STOP if it needs scraping. Σ=total per dimension; valuation untouched. |
