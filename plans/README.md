# Implementation Plans

Generated by the improve skill on 2026-06-26. Execute in the order below unless
dependencies say otherwise. Each executor: read the plan fully before starting,
honor its STOP conditions, and update your row when done.

## 🔄 Reconcile 2026-07-11 — read this first (supersedes the 2026-07-10 one below)

Verified against `main` @ `2f3ac9fc` (= origin/main, pushed; CI run pending
operator eyeball). Between the 2026-07-10 reconcile and now, the operator
merged PRs #14–#21 and this session merged the 151–155 batch.

**✅ Newly verified DONE (content-marker-checked on HEAD):**
- **150** security alerts (PR #21; `Math.random` grep 0, ci.yml `permissions:` present).
- **151–155** operator bug batch (this session; all markers present, 1009 tests,
  live-verified 153/154/155 — see rows).
- **125** SQLite recompute scoping — **index was stale, now corrected**: merged
  2026-07-10 via PR #15 (`3c5fc684`, "filter tombstones, write only changed
  balances, drop O(n²) asset scan"; marker at repositories.ts:4525).
- **141 UX follow-ups** (custom-asset discoverability + per-holding stale badge)
  — merged via PR #15 (`67064f06`).
- **138 tail (partial)** — PR #16 (`feat/ai-modalshell-tail-b`) migrated the
  accounts wizard drawer + goal-editor sheet to ModalShell.

**RESOLVED same day (2026-07-11 evening):** the "conflicting tail" alarm below
was a false positive — closer inspection showed PR #16 (`feat/ai-modalshell-tail-b`)
contained **11 commits**, a complete redo of ALL the tail overlays, already on
main; a repo-wide sweep found **zero** remaining raw (non-ModalShell) overlays.
`feat/ai-modalshell-tail` (tip `969d11c0`) and `docs/ai-reconcile-reality`
(tip `030c8fc8`) were both fully superseded and have been **deleted** (tips
recoverable via reflog), along with their stale worktrees. **138 is now fully
DONE, tail included.**

**✅ 146 MERGED** (same evening): merge commit on main after `2f3ac9fc`;
`seedToday?: string` on both recurring drafts, both `firstFutureRunDate`
callers pass it, 4 create sites set `seedToday: todayInTimezone(timezone)`
(RecurringRulesTab, RecurringInvestmentsTab, CashFlowRoute ×2); demoData/seed
correctly left on the UTC default; belt-and-braces test proves seed-vs-UTC
divergence under both repos (1009→1011 tests). No UI-store import in
`src/data/` (grep 0). **Closes plan 120's last deferred item — no recurring
path seeds from UTC anymore.**

**🔨 Executable now:** 142 (DCA spike), 143 (household spike). That's the
whole remaining backlog.

**✅ LANDED (2026-07-11 night):** `fix/ai-sitca-fund-symbol-collision`
(`33b9add2` + proxy fix `db31a02a`, from the operator's side session) was
reviewed, gate-verified on its tip (tsc 0 / 1015 tests), and **merged to main**
(post-merge: tsc 0 / **1017 tests** / lint 0). It found that **SITCA 基金代號
are NOT unique across 投信** (advisor-verified on the live CSV: 4,394 rows,
only 762 distinct codes — `DIO63` repeats ×24) — the old `byCode` map silently
clobbered ~3,600 funds. Canonical fund ticker is now `SITCA:<受益憑證代號>`
(unique file-wide); legacy `SITCA:<基金代號>` still prices only when
unambiguous (wrong price > missing price avoided by design); decision doc
corrected in docs/taiwan-fund-nav-plan.md. **Browser end-to-end verified after
the merge** (proxy now returns real data): `台積電` → `2330.TW`;
`兆豐美元` → `SITCA:T0123A/T0123B` (new cert-code symbols live). `T1605Y`
couldn't be live-tested only because SITCA's server was serving a truncated
~37-row file at that hour (direct curl confirmed upstream truncation; the full
file has 4,394 rows) — cert-code matching is unit-tested with the real row.
**Follow-up chip filed:** guard the 1-hour fund cache against truncated
upstream CSVs (row-count sanity floor) so a bad fetch doesn't pin near-empty
search/prices for an hour.

**🔒 Still deferred:** 132's vault-key rotation spike — note PR #15 also landed
`ade8e99d` "additive ECDH pairing helpers for vault-key exchange", which is
groundwork for exactly this; the spike is now cheaper than when deferred.

**⚠️ Only-operator:** unchanged (2-device pairing/revocation verification;
Tauri-app spot-check of 151/152 search; CI eyeball of the latest push).

---

## 🔄 Reconcile 2026-07-10 — superseded (kept for the record)

Verified against `main` @ `3c90361c` (local; `origin/main` @ `061603e4` — the tail
12 commits are in PR `ai/2026-07-10-ui-a11y`, adopting PR-flow going forward).

**This session (2026-07-09→10) executed the deep-audit batch 118–149.** Every merged
plan was reviewed as a diff + gate-verified; markers spot-checked present on `main`
and the pushed CI run (build/checks/tauri/worker/e2e) is **all green**.

**✅ MERGED & verified (31):** 118, 119, 120(partial), 121, 122, 123, 124, 126, 127,
128, 129, 130, 131, 132, 133, 134, 135, 136, 137(4/5), 138, 139, 141, 144, 145, 147,
148, 149 + the 2026-07-04 alpha.52/53 tail. Spot-checked markers all present.

**⏭️ REJECTED / superseded:** 140 (all 4 deliverables shipped independently — see its
row). 137-C (formatPercent) correctly skipped (ratio-vs-percent mismatch).

**🟢 Operator actions DONE (2026-07-10):** worker deployed + migrations 0004–0007
applied to prod D1; CSP `connect-src` pinned; pushed to GitHub → CI all green.

**🔨 Still executable (client-only, no deploy debt):**
- **142** DCA finish-or-retire (spike → `docs/dca-decision.md`)
- **143** Household sharing (design spike)
- **138 tail** ~10 remaining overlays → ModalShell, per-file
- **125** SQLite recompute scoping (now safe — 126 harness + 127 migration tests exist)
- **141 UX gaps** (custom-asset creation discoverability; per-holding stale badge)

**⚠️ Only-operator (can't be automated):** real-device verification of pairing +
device revocation (server logic has 25 worker tests + is deployed; needs 2 phones).

**🔒 Deferred security spike:** 132's vault-key rotation on revocation (a revoked
device that already captured ciphertext can still decrypt THAT; future data is cut
off by per-device auth). Needs 131's `/keys` machinery; substantial.

---

## 🔄 Reconcile 2026-07-03 (pre-release) — read this first

Verified against `main` @ `3d0bfdaf` (version `0.1.0-alpha.51`).

**Verified DONE & on main** (spot-checked by content marker): 101–115 all merged
and green (817 tests). The whole 2026-07-02→03 critique+follow-up batch shipped.

**⚠️ Executed-but-UNMERGED (would be MISSING from a release cut now)** — all three
merge **clean** into current main, each 1 commit ahead / 30 behind:
- **098** `fix/ai-quickadd-category-kind` (`c657dde5`) — QuickAdd chips filter by 收入/支出.
- **099** `fix/ai-tx-summary-follows-filters` (`c5ccd50b`) — tx summary cards follow filters (`transactionsSummary.ts` ABSENT on main = confirmed unmerged).
- **100** `fix/ai-updater-changelog-notes` (`75f024c7`) — updater `latest.json` notes + the **alpha.51 CHANGELOG section** (main's CHANGELOG top is still alpha.50).
- **090** `fix/ai-macos-window-drag` (`734c80e8`) — correctly stays unmerged (superseded by 094). Leave it.

**Release blockers found**:
1. Version is `alpha.51` in all 3 files but **CHANGELOG has no alpha.51 entry** on main
   (it lives on the unmerged 100 branch, and only covers 096–100 — NOT tonight's 101–115).
2. 098/099/100 not merged → they won't ship unless merged first.

**Outstanding (not release-blocking)**:
- **Manual/GUI verification** (headless can't do): macOS sidebar drag (111) + logo/notification (097); 隱藏金額 full-page mask (101); DRIP record (109); annual-tax expand + CSV (114); 現金同色 (112); bear amber line (113); GitHub Actions CI first run after next push (102).
- **115 Phase 2**: per-file inline-style cleanup, one file per dispatch. Ongoing.
  - ✅ **CashFlowRoute** done (merge `3146ea84`, 2026-07-04): 209→109 inline (100 static sites converted to Tailwind/`ns` utilities); extracted `.ns-modal-scrim`/`.ns-modal-panel` (3 identical modals); pixel-identity confirmed in preview (desktop `.ns-field-label` computed exact, drawer + mobile clean, no console errors); 826 tests green. Repo-wide inline count 2157→2047.
  - **Follow-up nit**: the new `.ns-modal-scrim` hardcodes `rgba(0,0,0,0.4)` (byte-copied from the old inline — pixel-identical, correct for a refactor). It could adopt `var(--ns-scrim)` (plan 107's canonical token) to align CashFlowRoute's 3 modals with the rest of the app — a small *visual* change, opt-in.
  - ✅ **InvestmentsAnalyticsTab** done (merge `60afa56a`, 2026-07-04): 174→126 (48 static sites converted). Executor hit session limit BEFORE committing — reviewer verified the uncommitted work: tsc/lint/826(its base)+831(main) tests, full-diff read (pure style→class, no logic), **exact spacing mappings checked** (gap-5/7=20/28px, mb-2/2.5/6, py-4 px-5, scroll-mt-16, gap-1), `ns-field-label` applied ONLY to `marginBottom:6` originals (8/10/4 → `mb-2`/`mb-2.5`/`mb-1`+`muted`), dynamic `borderLeft`/`bg-hover`/Recharts inline correctly preserved; committed on branch + merged clean; preview verified desktop + 375px (report/TWR-XIRR box + 103 warn annotation intact, no refactor-related console errors — the Base UI `nativeButton` warning on MetricHelp is pre-existing).
  - ✅ **DashboardRoute** done (merge `95929c80`, 2026-07-04): 157→94 (63 static sites converted). Reviewer verified: tsc/lint/831 tests, full-diff line-by-line (pure style→class, no logic), **exact mappings** (mb-1=4px/pt-4 px-5 pb-3/pt-1.5 px-2 pb-2/mt-1.5/gap-2.5/mb-4), `truncate` replaces only nowrap+hidden+ellipsis (no dropped maxWidth), new `.accent` class (`color:var(--ns-accent)`) applied at 4 STATIC-color sites (added to the DS `.pos/.neg/.muted` shorthand block), all dynamic (tone ternaries, kpi/mover colors, pct widths, odd px like 22/42/88, clamp fontSize, Recharts) correctly left inline. Preview-verified via DOM (cards positioned correctly, `.truncate`→ellipsis/nowrap, `.accent`→rgb(159,232,112)); top screenshot clean; no console errors (mid-scroll black screenshots were a preview-tool glitch, DOM confirmed content present).
  - ✅ **ALL remaining files done (2026-07-04→05)** — 22 more files cleaned in parallel waves (each executor: one file, no globals.css, static→Tailwind/existing-class only, dynamic/Recharts/odd-px left inline; reviewer verified every merge — gates + full-diff no-logic scan + exact-mapping spot-check + `ns-field-label`/`truncate`/`.accent` only on exact matches; several stalled executors' uncommitted work was reviewed + committed by the reviewer). Merged: FIRECalculatorRoute 97→60, ConnectSection 101→66, AccountsRoute 113→67, InvestmentsRoute 74→67, HoldingDetailRoute 78→45, InvestmentImportWizard 77→63, ReconcileRoute 67→42, CategoriesSection 69→47, RecurringRulesTab 62→51, GoalsRoute 61→51 (107/113 amber + depletion marker verified intact), TransactionDetailPanel 45→26, CategoriesTab 49→24, OnboardingOverlay 44→38, ManualPriceImportWizard 44→37, InvestmentsAddSheet 62→29, CategoriesRoute 52→39, AnnualReportRoute 34→32, RecurringInvestmentsTab 40→26, AppShell 41→32 (drag-region count 9=9 + titlebar-inset/sidebar-drag-cap untouched — 094/097/111 macOS machinery preserved), MerchantsTab 39→22, QuickAdd 38→35 (toLocaleString untouched). Also the modal-scrim follow-up landed (`.ns-modal-scrim` → `var(--ns-scrim)`, CashFlow modals now match the app). **Repo-wide inline style: 2157 → 1541 (~29% reduction).** `npx tsc`/`npm run lint` clean; 830/831 tests (the 1 failure is the date-flaky test below, not styling). The remaining ~1541 inline are legitimately dynamic (data-driven colors/widths), Recharts/SVG props, CSS-var-only values, or odd non-Tailwind-scale px — the intended floor.
- **⚠️ Date-flaky test (found 2026-07-05, NOT styling-related)**: `src/data/repositories.recurring.test.ts > postDueRecurringTransactions > "does not duplicate the seed transaction when a new monthly rule starts next period"` fails on `main` on 2026-07-05 (`expected +0 to be 1`). Cause: the test uses hard-coded seed dates (2026-06-05, monthly dayOfMonth 5) but `postDueRecurringTransactions` reads the REAL current date; on 07-05 (one month after the seed, same day-of-month) it crosses a period boundary and the assertion breaks. **Fix = freeze the clock in the test (`vi.setSystemTime(...)`)** so it's deterministic regardless of the calendar. Small test-hygiene fix; worth confirming the posting logic is correct on the 5th while there. Not caused by any style refactor (fails identically on pre-cleanup main).
- **114 dividend seam** (index-noted): year-total dividends vs per-holding scan — consistent for the real route, optional future unify.
- ~~手續費/證交稅 split~~ — **RETIRED 2026-07-04 (operator: not needed)**: TW personal tax does not require reporting brokerage commission or securities-transaction tax (證所稅 currently suspended), so splitting the single `fee` field serves no filing purpose. Won't do.


## Execution order & status

| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 001  | Fix progress bar layout thrashing | P1 | S | — | DONE (Verified) |
| 002  | Replace ns-eyebrow with proper typographic hierarchy | P2 | M | — | DONE (Verified) |
| 003  | Reduce cognitive load in InvestmentAddSheet form | P1 | S | — | DONE (Verified) |
| 004  | Unify Add Icons & Optimize Cash Flow Layout | P2 | S | — | DONE (Verified) |
| 077  | Apple-native polish (macOS+iOS) — RWD, sync, native feel, extensions, on-device AI | P1 | L | — | PARTIALLY DONE via slices 079–084 (see breakdown); deferred sub-items remain (listed below) |
| 078  | React Native mobile feasibility assessment (reference doc) | P2 | — | — | DONE (decision: stay with Tauri mobile) |
| 079  | macOS native-feel polish — title bar, app menu, Dock badge, window state | P2 | M | — | ✅ MERGED on main (verified 2026-07-02: `data-native-glass` in AppShell.tsx on main; macOS GUI still needs a manual eyeball) |
| 080  | Foundation Models: pass category list through to Tier 1 (completes 077 Phase 7 data path) | P1 | S | — | ✅ MERGED on main (verified 2026-07-02: `src/lib/foundationModels.ts` on main) |
| 081  | Sync cursor/identity durability via file mirror (077 Phase 3.1, corrected) | P1 | M | — | ✅ MERGED on main (verified 2026-07-02: sync client + README on main) |
| 082  | Local notifications for due reminders (077 Phase 6.1) | P2 | M | — | ✅ MERGED on main (verified 2026-07-02) |
| 083  | Collapse the few non-responsive inline grids at 390px (077 Phase 1–2, scoped) | P2 | S | — | ✅ MERGED on main (verified 2026-07-02; 390px eyeball still worthwhile) |
| 084  | Fix mobile bottom-nav tap target (z-index) + overflowing net-worth period strip | P1 | S | — | ✅ MERGED on main (verified 2026-07-02; live 379px nav re-test still worthwhile) |
| 085  | SwiftUI Widget & App Intents — design/scaffold spec (decision doc, no dispatch) | P3 | S | — | DONE (design doc; awaiting your scope/account decision before Widget/App-Intents impl) |
| 086  | Dependency license audit — confirm no copyleft, document (077 Phase 5) | P2 | S | — | ✅ MERGED on main (verified 2026-07-02: `license:check` script on main; deps clean; project intentionally GPL-3.0+CLA per 072) |
| 087  | Verify & document power/perf budget (077 Phase 7.5) | P2 | S | — | ✅ MERGED on main (verified 2026-07-02: `docs/performance-budget.md` on main) |
| 088  | On-device AI: auto-categorization + monthly summary (077 Phase 7.2, spec) | P3 | M | — | DONE (spec; 089 Feature B built; Feature A auto-categorization still TODO). Swift IS compile-gated via `check:tauri`→build.rs→swiftc; only model TEXT quality needs a device |
| 089  | On-device monthly summary card (088 Feature B) | P3 | M | — | ✅ MERGED on main (verified 2026-07-02: `src/domain/monthlySummary.ts` on main; released in v0.1.0-alpha.47) |
| 090  | Fix macOS window dragging — full-width top drag strip (079 regression) | P1 | S | — | REJECTED / SUPERSEDED by 094 — verified 2026-07-02 that its unique commit `734c80e8` is NOT on main (correct); do NOT merge `fix/ai-macos-window-drag` |
| 091  | Make Taiwan SITCA funds findable in ticker search | P1 | M | — | ✅ MERGED on main (verified 2026-07-02: commit `e47e7040` on main) |
| 092  | Resolve Chinese names for Taiwan ETFs (search shows English) | P2 | M | — | ✅ MERGED on main (verified 2026-07-02: commit `a725b014` + taiwanMarketDataProvider changes on main; the 2026-06-29 "not on main" warning is now resolved) |
| 093  | Fix sync crash on duplicate recurring_occurrence_key (cross-device dedup) | P1 | M | — | ✅ MERGED on main (verified 2026-07-02: commit `b809449d` on main; the 2026-06-29 "message-only merge" warning is now resolved) |
| 094  | macOS window drag — grant `core:window:allow-start-dragging` (+ full-width strip) | P1 | S | — | ✅ MERGED on main (verified 2026-07-02: `allow-start-dragging` in capabilities/desktop.json on main; SUPERSEDES 090) |
| 095  | Notification center + acknowledge (makes the Dock badge clearable) | P2 | M | — | ✅ MERGED on main (verified 2026-07-02: `src/components/NotificationCenter.tsx` on main) |
| 096  | Make imported-holding opening lots permanently cash-neutral (edit path leaks a 交割 ledger row) | P1 | M | — | DONE (executed + reviewed 2026-07-02; branch `fix/ai-opening-lot-cash-leak` off `b33bf55e`, unmerged; 803 tests / build / lint re-verified by reviewer; one approved deviation: SQLite update now writes `cashless` column explicitly) |
| 097  | Sidebar clearance below macOS traffic lights + unclip notification panel | P1 | S | — | ✅ MERGED on main (operator-authorized merge `e5c61291`, 2026-07-02 evening; content-verified: `--ns-titlebar-inset` in globals.css + anchorRect in NotificationCenter; tsc/lint/803 tests green post-merge; macOS visual check still pending operator) |
| 098  | Honor category 收入/支出 kind in Quick Add picker | P2 | S | — | DONE (executed + reviewed 2026-07-02; branch `fix/ai-quickadd-category-kind` off `b33bf55e`, unmerged; 802 tests / build / lint re-verified; remaining `categoryGroups.map` at QuickAdd.tsx:126 is the NL-parser ctx, correctly out of scope) |
| 099  | 交易紀錄 summary cards follow active filters; exclude 匯入 baselines from 總買入 | P2 | S | 096 (DONE, merged) | DONE (executed + reviewed 2026-07-02; branch `fix/ai-tx-summary-follows-filters` off `b33bf55e`, unmerged; 803 tests / build / lint re-verified; merge into main verified conflict-free and preserves 096's 複製 change) |
| 100  | Show changelog in the in-app updater (populate `latest.json` notes from CHANGELOG) + add alpha.51 CHANGELOG section | P2 | S | — | DONE (executed + reviewed 2026-07-02; branch `fix/ai-updater-changelog-notes` off `b33bf55e`, unmerged; `bash -n`/changelog-extract/NOTES_OK/build re-verified; merge into main verified conflict-free) |
| 101  | Route Goals/FIRE/Settings money displays through currency helpers (privacy-mask bypass, 實測確認) | P0 | S | — | ✅ MERGED on main (fast-forward to `6c20116a`, 2026-07-02 evening; tsc/807 tests re-verified post-merge; approved extension: 3 additional display-only NT$/M literals in FIRECalculatorRoute fixed per the plan's own Step 2 clause; 遮罩開啟的目視驗收待 operator) |
| 102  | Lint guardrail (ban `toLocaleString` in routes) + CI test/lint gate | P1 | S | 101 | ✅ MERGED on main (fast-forward to `9f26acf3`, 2026-07-02 evening; lint/tsc re-verified post-merge; reviewer ran own negative probe — rule fires and reverts clean; first real CI run triggers on next push, operator to eyeball Actions) |
| 103  | Number credibility guardrails — XIRR annualization annotation, hero % baseline floor, hero never truncates | P1 | M | — | ✅ MERGED on main (merge `184e0ac7`, 2026-07-02 evening; executed + reviewed; new domain `changePctWithFloor` + 6 tests (813 total), 3 pct sites guarded, Badge tolerates null, hero ellipsis removed (flexShrink 0 + parent wrap), XIRR sub-label warns「年化自 N 天紀錄推算」when span<365; XIRR_MIN_DAYS gate & all calculations untouched; hero 換行/warn 色視覺目視待 operator) |
| 104  | Settings 分類表「已消費」接真實資料（目前寫死 0） | P2 | S | — | ✅ MERGED on main (fast-forward to `d63c15e1`, 2026-07-02 evening; executed + reviewed; canonical categoryPeriodSpend wired for current month, mock `spent=0` and hardcoded `NT$0` both removed; tsc/lint/807 tests re-verified post-merge; 兩頁對帳目視待 operator; known minor: 已消費/預算欄 label 寫死 "TWD" 而非 primaryCurrency — 與 101 的預算欄一致，若未來支援非 TWD 主幣別需一併改) |
| 105  | 卡片標題中文化（"1 active goal" / "Top Movers" / "Category" 表頭） | P2 | S | — | ✅ MERGED on main (merge `e9ca16a3`, 2026-07-02 evening; executor branched off stale `479b6256` by mistake — regions didn't overlap, merge clean, and the authoritative re-verification ran post-merge on main: tsc/lint/813 tests/criteria greps all green; copy.csv 實際路徑是 src/locales/copy.csv（計劃筆誤），_meta.json 為 copy:export 的必然副產物一併入 commit) |
| 106  | 手機版 Dashboard 頁首收斂 + `.ns-hscroll` 捲動提示 | P2 | S | — | ✅ MERGED on main (fast-forward to `3706f08a`, 2026-07-02 evening; correct baseline; `sm:contents` wrapper + right-edge mask fade + `--nofade` escape class; inventory confirmed `.ns-hscroll` has exactly one consumer (dashboard strip); tsc/lint/813 tests re-verified; 375px 目視由 reviewer preview 補驗) |
| 107  | 圖表語意收尾 — 悲觀情境琥珀色、FIRE 耗盡標註、`--ns-scrim` token、產業濾器去雜訊 | P3 | M | — | ✅ MERGED on main (merge `f6682af9`, 2026-07-03; executor hit session limit mid-run, resumed via coordinator and finished all four items; approved deviation: `--ns-scrim` ×3 not ×2 — globals.css 的主題 token 實際有三個區塊（light / prefers-dark / data-theme=dark），計劃寫 2 是 advisor 誤判; follow-up noted: GoalsRoute `chartColor` 在選「保守」情境時主線仍用 `--ns-neg` 紅——與本計劃 A 項同類、當時未列入 scope，值得小修) |
| 108  | 投資輸入支援小數點後 5 位（價格 decimals 2→5、股數 4→5、tooltip 去釘死） | P1 | S | — | ✅ MERGED on main (`a22e6792`, 2026-07-02 evening; executed + reviewed same day; tsc/lint/804 tests re-verified post-merge; one approved deviation: plan's `toFixed(2)` done-criterion grep was overbroad — 4 remaining matches are pre-existing percentage formatters, correctly out of scope) |
| 109  | DRIP 金額自動帶入 + 驗證容差放寬（1e-6 → max(1, 0.1%·A)） | P1 | M | 108 | ✅ MERGED on main (fast-forward to `a577986c`, 2026-07-02 evening; tsc/807 tests re-verified post-merge; approved deviations recorded: overbroad `0.000001` done-criterion grep — 2 remaining hits are pre-existing oversell checks; auto-fill 手動 UI 流程待 operator 目視) |
| 110  | 通知入口移到總覽右上角（鈴鐺 + 未讀 badge，面板向下展開） | P2 | M | 097, 106 (both merged) | ✅ MERGED on main (merge `b7dd5ba5`, 2026-07-03; icon-only outline bell in dashboard header, badge top-right, panel opens below/right-aligned `min(340px, 100vw-32px)`, sidebar entry removed; reviewer live-verified in preview: bell present, sidebar clean, panel below-button right-aligned within viewport with real reminder content; 手機也因此首次有通知入口) |
| 111  | macOS sidebar 頂部整區可拖曳（drag-region 補子元素 + 留白蓋板） | P2 | S | — | ✅ MERGED on main (merge `36fffb46`, 2026-07-02 evening; executor stalled once pre-commit, resumed via coordinator message and finished clean; +4 drag-region on non-interactive children, `.ns-sidebar-drag-cap` (macOS-gated, z:1) covers top whitespace; z-index 30 strip untouched per guardrail; tsc/lint/813 tests re-verified; **macOS 拖曳 4 點實測清單待 operator**) |

## 2026-07-03 follow-up batch (112–115) — from the deferred/critique tail

Planned against main @ `b7dd5ba5`. Operator decisions: #1 現金同色 only (not full palette unify); #2 年度報表往**報稅深化**（not year-in-review); #3 **手動清舊 inline style**; #4 fix the 107 tail. All four independent; suggested order 113 → 112 → 114 → 115.

| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 112  | 資產配置圓餅「現金」釘死 chart-2、與 KPI 卡同色 | P3 | S | — | ✅ MERGED on main (merge `bb1ebdc0`, 2026-07-03; allocation memo pins 現金→chart-2, other categories keep index colors, line-482 map untouched; tsc/lint/813 tests re-verified; 現金兩處同藍的目視待 operator) |
| 113  | 目標頁「保守」情境主線改琥珀（107 尾巴） | P3 | S | — | ✅ MERGED on main (fast-forward to `26ff144b`, 2026-07-03; one-line `chartColor` bear → `--ns-chart-3`; `ns-neg` count 5→4 (pace-behind + delete-button reds untouched); tsc/lint/813 tests re-verified) |
| 114  | 深化年度報稅工具 — 逐檔明細、境內/海外分列、CSV 匯出 | P2 | L | — | ✅ MERGED on main (fast-forward to `5398b877`, 2026-07-03; executed + reviewed; `AnnualReportYear` gains byHolding/domestic/overseas, 4 new domain tests verify conservation (Σ byHolding === year total) + TW/US split + null→overseas + sort; route gains expandable per-holding rows + 境內/海外小計 + CSV export; `realizedByYearForAsset`/`buildDividendAnalysis` internals untouched; 817 tests green; 手續費/證交稅拆分刻意留為後續 schema 決定. **Reviewer-noted seam** (not a blocker): year-total `dividends` still comes from caller's `dividendByYear` aggregate while byHolding.dividends is a per-record scan — consistent for the real route (same records), could diverge only for a synthetic caller; unifying the two dividend sources is a possible future cleanup) |
| 115  | 樣式系統裁決規則 + 分階段清理 inline style | P3 | L (phased) | — | 🔶 PHASE 1 MERGED on main (fast-forward to `3d0bfdaf`, 2026-07-03; rule added to AGENTS.md + DESIGN.md §12.8; `.ns-field-label` extracted; 10 exact-match sites migrated (of 126 candidates — the other ~28 with marginBottom:6 carried extra props and were correctly left for Phase 2); all migrations are `text-xs div` → pixel-identical; inline count 2157→2147; tsc/lint/817 tests green). **PHASE 2 = TODO**: per-file inline cleanup, highest-count first (CashFlowRoute 211 → InvestmentsAnalyticsTab 174 → …), one file per dispatch — ongoing, dispatch when you want it. |

Notes:
- 114 explicitly does NOT split 手續費/證交稅 (single `fee` field = schema change, flagged as a separate future decision). It deepens along the tax axis the operator chose.
- 115 Phase 1 (rule + `.ns-field-label` extraction) is a standalone win; Phase 2 is an ongoing per-file cleanup, one file per dispatch — not a single-shot job (2144 inline styles across ~20 files).

## ✅ Merge-state reconciled (2026-07-02) — supersedes the 2026-06-29 discrepancy

The 2026-06-29 warning (090–093 "not on main", message-only merges) is **RESOLVED**. Re-verified
against `main` @ `afef92ef` (version `0.1.0-alpha.50`) using real commit + file-content checks
(`git merge-base --is-ancestor` alone is NOT trusted — a `-s ours` merge can make a branch an
ancestor without its code; every claim below was confirmed by `git show main:<file>` / a distinctive
code marker actually present on main):

- **Everything through 096 is genuinely on `main`**: 079–089, 091, 092, 093, 094, 095, 096. Each was
  confirmed by a distinctive file or commit present in main's tree (e.g. 092 `a725b014` +
  `taiwanMarketDataProvider` changes; 093 `b809449d`; 094 `allow-start-dragging` in
  `capabilities/desktop.json`; 096 the `cashless flag is a stored property` comment ×2). The operator
  redid the earlier message-only merges properly.
- **090 is correctly NOT on main** — superseded by 094; its unique commit `734c80e8` is absent. Leave it.
- **Only 097, 098, 099 remain unmerged** (this session's work), each on its own branch, each
  re-verified build/lint/tests green and merge-clean into main. See rows above.

Housekeeping: ~~stale agent worktrees~~ **CLEANED 2026-07-02 evening (operator-authorized)**:
all 23 `.claude/worktrees/agent-*` worktrees removed (9 clean + 14 whose only dirt was
`package-lock.json` / `Cargo.lock` / `.claude/settings.local.json` session junk — verified
file-by-file before `--force`). All branches retained; `claude/blissful-swartz-6d8d07`
worktree left untouched (not an agent worktree, likely another Claude session's).
Merged branches (`fix/ai-*` for 079–097, 108) can be deleted at leisure with `git branch -d`.

## 2026-07-04 batch (116–117) — AI summary + updater notification polish

Planned against main @ `4c22f478` (after v0.1.0-alpha.52 release + CashFlowRoute style cleanup + modal-scrim `--ns-scrim` fix). From operator feedback on the alpha.52 build.

| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 116  | AI 本月摘要移到問候語下方、去卡片化、渲染 markdown | P2 | M | — (creates shared `MarkdownText`) | ✅ MERGED on main (`27d67be7`, 2026-07-04; executor hit session limit BEFORE committing — reviewer verified the uncommitted work (tsc/lint/831 tests + full diff + 5 MarkdownText unit tests + live: no console errors, bottom card removed, bold regex sanity-checked), committed on its branch, merged clean; `MarkdownText.tsx` created (no-dep `**bold**`/`##`/`-` renderer); summary now inline under greeting, de-carded, 重新產生 is icon-only + privacy note in tooltip. **On-device FM summary can't render in the browser dev shell — operator must eyeball the real immersive summary + markdown on macOS.**) |
| 117  | 更新通知預設不顯示 changelog（展開才看）+ markdown 渲染 | P2 | M | 116 (shares `MarkdownText`) | ✅ MERGED on main (`ec33588d`, 2026-07-04; executor committed cleanly this time; `ToastDescriptor` gains `details?: string` + a「更新內容 ▾/▴」toggle rendering `MarkdownText` (maxHeight 200 scroll) only when expanded; update toast now shows fixed one-liner + 立即更新 by default, changelog behind the toggle, 140-char truncation removed; reused 116's `MarkdownText`; tsc/lint/831 tests; settings.local.json correctly NOT committed. **Real updater toast appearance is Tauri-only — operator eyeballs on macOS; MarkdownText bold/heading rendering already unit-tested.**) |

Shared insight: the raw `**bold**` / `###` markdown the operator saw in BOTH the AI
summary and the update toast is the same root cause — no markdown renderer. 116 creates
a small no-dependency `MarkdownText` component; 117 reuses it. The on-device model already
gets a "2–3 句, no markdown" instruction but ignores it, so the fix is to *render* the
markdown, not to re-instruct. "更簡短" via the Swift prompt (`FoundationModels.swift:247`)
is an optional on-device follow-up (device-verify only), noted in 116, not in JS scope.

> **Execution log (2026-07-09).** Merged to main so far this session: 118, 144
> (new — from 124's audit), 123, 124. Main advanced `65fe04c1` → `1c64e00d`,
> 843 tests green. **Harness note for future `execute` runs:** agent worktrees
> are created from the session's ORIGINAL commit, not live `main` — so any plan
> depending on freshly-merged work must have its executor run
> `git merge --ff-only main` in the worktree BEFORE editing (124 re-STOPPED
> twice until this was done). Merged agent branches (`fix/ai-fee-autofill-*`,
> `fix/ai-accounts-recalc-invalidation`, `fix/ai-fx-converter-perf`,
> `fix/ai-query-staletime`) and their `.claude/worktrees/agent-*` dirs can be
> cleaned up at leisure. Optional follow-up (plan 124 maintenance note, not
> done): add a one-line AGENTS.md gotcha that new mutations must invalidate
> their query keys now that `staleTime: Infinity` no longer masks omissions.

## 2026-07-09 deep-audit batch (118–143)

Source: `/improve deep` + `/impeccable` lens, planned against `main` @ `65fe04c1`
(v0.1.0-alpha.53, 831 tests green). 8 parallel category audits (correctness ×2,
security, perf, tests+DX, debt+deps, docs+direction, UI/UX); every finding
vetted against the code before planning. Operator selected ALL eight clusters.

**Suggested execution order** (independent unless noted): quick wins first
(118, 123, 124, 135), then test foundation (126–128) before the risky perf/
refactor work (125), security in sequence (130 → 131 → 132; 133 anytime),
everything else parallel-friendly.

| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 118 | Fee auto-fill preserves stored fee on TW edit | P1 | S | — | ✅ MERGED on main (merge `deef1703`, 2026-07-09; commits `b2a8e695`+`1c246f38`). Helper in leaf module `investmentsAddSheetFee.ts` + 4-case test; post-merge main re-verified tsc 0 / 835 tests / lint 0. |
| 119 | Repo parity: recurring nextRunDate advance + browser sync occurrence dedup | P1 | M | — | ✅ MERGED on main (fast-forward `5f5c3aa6`, 2026-07-09; commits `86b5fe6a`+`5f5c3aa6`). Bug1: browser advances `nextRunDate` on `advanced.size>0` (recompute still gated on `posted>0`). Bug2: browser `applySyncChanges` applies the SQLite winner rule (`sort()[0]`, loser tombstoned with `bump()`). +3 tests (fail-first). Recurring-investments path checked — no bug1 pattern (single-rule post). Post-merge main: tsc 0 / 846 tests / lint 0. Removes 2 divergences plan 126 would flag. |
| 120 | Timezone-aware "today/current month" sweep (4 high-impact sites) | P2 | S | — | ✅ MERGED (partial) on main (fast-forward `ae0c9ce1`, 2026-07-09; commit `ae0c9ce1`). 3 user-facing sites fixed (Dashboard month KPI, budget currentMonth, CashFlow UpcomingPayments) + `firstFutureRunDate` made injectable + 6 tests. **Repo-caller site legitimately STOPPED** (data layer has no timezone — validated) → deferred to plan 146. Post-merge main: tsc 0 / 852 tests / lint 0. |
| 145 | CashFlow per-row FX uses the daily-rate index (plan-123 follow-up) | P2 | S | 123 (merged) | ✅ MERGED on main (fast-forward `1330280e`, 2026-07-09; commit `1330280e`). Both per-row daily `convertCurrency` calls now pass `dailyRateIndex: fxIndex` (memoized on `fxHistory`); grep→0; bit-identical per 123's proof. Post-merge main: tsc 0 / 852 tests / lint 0. |
| 146 | Recurring-seed timezone plumbing (finishes 120's deferred repo-caller piece) | P3 | S | 120 (merged) | ✅ MERGED on main (2026-07-11 evening; commit `e943963b`). `seedToday?` on both drafts, both callers pass it, 4 UI create sites set it, demoData left on UTC default; +2 tests (seed-vs-UTC divergence, both repos; 1011 total). Reviewer re-ran tsc/tests/greps in the worktree; post-merge main tsc 0 / 1011 tests / lint 0. Closes 120's last deferred item. |
| 121 | FX missing-rate visibility (toPrimaryOrNull + annual-report warning) | P2 | M | — | ✅ MERGED on main (fast-forward `fb738b2c`, 2026-07-09). Additive `toPrimaryOrNull` (toPrimary byte-identical); dividendAnalysis/annualReport/valuation now exclude+count unpriced legs (`fxMisses`/`fxMissCount`); AnnualReportRoute shows 「有 N 筆配息因缺少匯率未計入」 (`--ns-warn`). +5 tests. tsc 0 / 957 tests / lint 0. (Deferred: Dashboard/Investments hero miss-signal.) |
| 122 | Small-fix basket: CSV error path / update-listener StrictMode / scope-edit tx / oversell clamp | P2 | M | — | ✅ MERGED on main (merge `f79ec930`, 2026-07-09). **A** CSV import wrapped in try/catch → zh-TW error toast. **B** update-check focus listener registers unconditionally (survives StrictMode; `checkForUpdate` self-throttles 6h). **C** SQLite `applyRecurringScopeEdit` override wraps the「全部」edit in one `withTransaction` (re-entrant via txDepth) → atomic, no half-apply; sqlite-gated fail-first test. **D** moving-avg oversell uses `soldQty` for proceeds+decrement (was inflating XIRR); fail-first test (realized 190 not 1090). tsc 0 / 960 tests / lint 0. |
| 123 | FX converter useMemo + per-pair rate index + CashFlow aggregate memo | P1 | S | coordinate w/ 121 | ✅ MERGED on main (merge `bd2292c2`, 2026-07-09; commits `0aebf545`+`9c00fdb0`). Per-pair binary-search index proven bit-identical (tie-break walk-back + 50×20 property test); 4 route sites memoized, 5th correctly excluded (plain helper already inside a useMemo); CashFlow aggregates memoized off searchQuery. Post-merge main: tsc 0 / 843 tests / lint 0. |
| 124 | QueryClient staleTime Infinity + no focus refetch | P1 | S | 144 (merged) | ✅ MERGED on main (fast-forward `1c64e00d`, 2026-07-09). `QueryClient` now `staleTime: Infinity` + `refetchOnWindowFocus: false` + `retry: 1`. Guarded by the full Step-1 invalidation audit (mutations + sync-apply all invalidate; 144 closed the one gap; market-data has its own focus listener). Post-merge main: tsc 0 / 843 tests / lint 0. Took 3 executor attempts (2 lost to the worktree-base quirk + an infra outage). |
| 144 | Fix missing `assets` invalidation in AccountsRoute.recalculate (124 prerequisite) | P1 | S | — | ✅ MERGED on main (fast-forward `abf17052`, 2026-07-09). `accounts.refetch()`→`queryClient.invalidateQueries()` matching GeneralSection; post-merge main re-verified tsc 0 / 835 tests / lint 0. |
| 125 | Scope the SQLite recompute (tombstone filter, changed-rows only, kill O(n²)) | P2 | M | 126 preferred | ✅ MERGED on main (PR #15 `918b18f6`, commit `3c5fc684`, 2026-07-10 — operator-driven, executed outside this index; index reconciled 2026-07-11). Marker verified: repositories.ts:4525 "Only write accounts whose balance actually changed". 126/127 safety nets were in place first, as the plan required. |
| 126 | Dual-repo test harness (money suites × memory+SQLite) | P1 | L | 119 first | ✅ MERGED on main (fast-forward `af2e94a0`, 2026-07-09). `describeEachRepo` harness (node:sqlite shim + snapshot-seed exercising real SQLite SQL); all 8 money suites run ×2 → 852→915 tests, ~5s. Found 1 real SQLite-only bug (boolean cols return 0/1 not JS bool) → plan 147, pinned live (not skipped). 119's fixes confirmed present. tsc 0 / lint 0. |
| 127 | Legacy-schema migration + repair-chain tests | P1 | M | — | ✅ MERGED on main (fast-forward `13370c71`, 2026-07-09). New `repositories.migration.test.ts` (311 lines, 4 tests): schema upgrade (PRAGMA re-adds dropped cols), cash-leak repair (bad leg tombstoned, balance restored), asset merge (split collapses to manual survivor), and idempotency (2nd `initialize()` byte-identical). No repair DIVERGENCE found (initialize is idempotent as designed). Reuses 126's shim; no production change. 915→919 tests; tsc 0 / lint 0. |
| 147 | Hydrate SQLite boolean columns to real booleans (bug found by 126) | P1 | S–M | 126 (merged) | ✅ MERGED on main (fast-forward `1b46c7bd`, 2026-07-09). `toBool` helper hydrates the 2 raw mappers (`isReviewed` ledger+investment, `cashless`); other 5 boolean cols already hydrated. Insert side stores `Number(bool)`=1 (round-trips). 126's divergence pin → true parity assertion. tsc 0 / 921 tests / lint 0. |
| 128 | CI covers build surface (vite build / check:tauri / worker / e2e smoke) | P1 | S–M | — | ✅ MERGED on main (fast-forward `4ba7784b`, 2026-07-09). ci.yml now has 4 parallel jobs: `checks` (unchanged) + `build` + `tauri` (rust-toolchain + rust-cache + release.yml's apt deps) + `worker` (standalone tsc). All 4 locally verified green (build, worker tsc, cargo fmt+check). **e2e job deferred** → the single smoke test is stale (expects native `<select>` gone since the COSS migration ~alpha.3; NOT a regression — confirmed pre-existing) → plan 148. First real CI run: operator to eyeball the Actions tab. |
| 148 | Un-stale the e2e smoke test + add the e2e CI job (completes 128; closes audit TEST-04) | P2 | S–M | 128 (merged) | ✅ MERGED on main (merge `a18c4468`, 2026-07-09). Fixed 4 layers of staleness (native-`<select>`→COSS filter buttons, quick-add placeholder copy, category-chip bullet, headings 投資/一般與備份) — the spec hadn't run since ~alpha.3; added onboarding-overlay + market-data test isolation (same pattern as the Yahoo mock); flow (empty state → filters → ⌘N quick-add → recompute) intact. `e2e` CI job added (chromium+webkit, timeout 15). tsc 0 / 952 app tests / e2e 2 passed local. **Closes audit TEST-04.** Operator eyeballs the first CI e2e run. |
| 129 | Sync orchestration + worker endpoint tests; pull N+1 batch fetch | P2 | M–L | — | ✅ MERGED on main (fast-forward `f43b073e`, 2026-07-09). Client tests: sync-manager (mutex/round-trip/error), push, conflictSummary (17). **Worker test harness built** (`@cloudflare/vitest-pool-workers` in workerd + D1) — 25 endpoint tests that RETROACTIVELY cover 131/132: revoked-device→401, set-once provision guard (409), malformed/mismatch tokens→401, per-user envelope scoping, cursor order, de-dup, device-id stamping. CI worker job now runs `npm test`. Step 5: `getSyncPayloads` batch kills pull's N+1 (pull.test.ts unchanged+green = semantics lock). app tsc 0 / 952 tests / worker tsc 0 / 25 worker tests. **Closes the "worker has no CI test" gap flagged on 131/132.** |
| 130 | Stronghold cutover completion (vault key + keypair; docs truth-sync) | P1 | M | — | ✅ MERGED on main (merge `6010c0b8`, 2026-07-09). vault.ts/pairing.ts route through a memoized `getSecretStore()` (Stronghold on device, localStorage fallback in web/tests; migration runs once, no race); key strings unchanged, zero crypto change; docs/comments reconciled (secret-storage-plan/DEVELOPMENT/ROADMAP). tsc 0 / 921 tests / lint 0. **Operator: run the per-platform verification checklist in docs/secret-storage-plan.md §4 before the localStorage-clearing follow-up.** Surfaced 2 follow-ups → plan 149. |
| 149 | Device reset wipes the SecretStore + account.ts uses the shared store (130 follow-ups) | P1 | S | 130 (merged) | ✅ MERGED on main (fast-forward `42508db3`, 2026-07-09). `reset.ts` now removes every `SECRET_KEYS` entry (incl. 132's device secret) from `getSecretStore()` — device reset truly wipes secrets on Stronghold too; account.ts consolidated to the single shared store (createSecretStore→getSecretStore, 0 left). tsc 0 / 935 tests / lint 0. Closes the gap 130 opened. |
| 131 | Pairing via ECDH key exchange (vault key never code-protected on relay) | P1 | M–L | 130; 129 rec. | ✅ MERGED on main (fast-forward `df813095`, 2026-07-09). Joiner B publishes only its ECDH public key; approver A wraps the vault key + account to B's key via `/keys` envelopes (never in a relay bundle — e2e-asserted). SAS fingerprint over the public key; per-session salt. New `POST /pairing/join` + `X-Pairing-Token` (hashed, device-scoped, 10-min TTL, single-use, defense-in-depth) + migration 0004; legacy path kept `@deprecated`. Standard Web Crypto only. tsc 0 / 928 tests / worker-tsc 0 / lint 0. **⚠️ OPERATOR: deploy the worker + run migration 0004, then verify pairing on 2 real devices BEFORE cutting a release (the client UI now drives the ECDH flow). Worker handlers have NO CI test (129 gap). Old↔new-version devices can't pair during the transition (alpha-OK).** |
| 132 | Per-device credentials — revocation actually revokes | P1 | L | 130, 131, 129 | ✅ MERGED on main (fast-forward `daf3613f`, 2026-07-09). Each device gets its own secret (only the SHA-256 hash stored server-side; plaintext never leaves the device — e2e-proven). `authenticate()` does device-token auth (revoked = hard-deleted row → 401, no oracle) + legacy account-secret fallback (deprecated). Push stamps the authed deviceId. Self-provision `POST /devices/:id/credential` is account-owned + set-once (409, no oracle). Migration 0005. tsc 0 / worker-tsc 0 / 935 tests / lint 0. **⚠️ OPERATOR: deploy worker + migration 0005, verify device matrix (new / joining / existing self-provision / revoke→401). Worker has NO CI test (129 gap).** **Deferred (real remaining gap): vault-key ROTATION on revocation** — this cuts off a revoked device's *relay* access, but a device that already captured ciphertext + still holds the shared vault key can decrypt THAT data; full crypto revocation (rotate + re-wrap via /keys) is a substantial spike needing 131's machinery. |
| 133 | Worker + shell hardening (per-user relay_sequence, rate limits, CSP, SQL caps) | P2 | M | 129 rec. | ✅ MERGED on main (fast-forward `bcb9314c`, 2026-07-09). **A** per-user relay_sequence (migration 0006: global→per-user unique index; scoped MAX; existing cursors survive) + 3 tests. **B** D1 fixed-window rate limiting (migration 0007; /users 10·min, pairing 30·min; fail-open on CF-Connecting-IP) + 5 tests. **C** CSP: STOPPED honestly — the only webview external fetch is the sync worker (build-time `VITE_NORTHSTAR_SYNC_WORKER_URL`, not in repo); **operator must pin `connect-src` to the real worker origin(s)**. **D** SQL caps: reviewed, `sql:allow-execute` is required (repo uses `db.execute`), no narrower grant — no change. worker tsc 0 / 33 tests. **⚠️ OPERATOR: deploy migrations 0006+0007 WITH the code (A relies on the per-user index); pin the CSP origin (C) + Tauri smoke.** |
| 134 | gain/loss semantic tokens on investment P&L (HoldingDetail hero, Analytics) | P2 | M | — | ✅ MERGED on main (fast-forward `a508dd59`, 2026-07-09). Badge `gain`/`loss` variants; globals.css bridge is single-source (`--gain/--loss → --ns-gain/--ns-loss`, which carry per-theme + per-`[data-gainloss]`), so one alias covers all modes (corrected the plan's 3-block assumption). HoldingDetail hero + 3 Analytics ternaries → gain/loss; Sortino/Drawdown accents, volatility markers, dividend-count badge, destructive buttons correctly LEFT (documented judgment list). tsc 0 / 960 tests / lint 0. (Noted future: DashboardRoute FX-rate changePct uses pos/neg.) Operator: eyeball TW palette on those screens. |
| 135 | aria-labels for ~28 icon-only buttons | P2 | S | — | ✅ MERGED on main (merge `d788440a`, 2026-07-09; commit `d5a4aa54`). 28 icon-only buttons + 2 raw month-picker arrows labeled (zh-TW, handler-derived: 關閉/取消/新增/編輯/刪除/確認…); grep→0; 16 .tsx files, attribute-only. Post-merge main: tsc 0 / 846 tests / lint 0. |
| 136 | Scrim/shadow token adoption (~12 overlays, drop decorative blur) | P3 | M | — | ✅ MERGED on main (fast-forward `d97abcea`, 2026-07-09). 12 overlay files: scrims → `var(--ns-scrim)` (inline where custom z-index/positioning; `.ns-modal-scrim` class only for ReconcileRoute's clean matches — the class bakes z:1000 which would push the scrim over z-998/999 panels elsewhere), decorative `blur(4px)` dropped from all 5 sites, drawer shadows → `--ns-shadow-2`. `rgba(0,0,0,0.3–0.6)` sweep → 0; only OnboardingOverlay's sanctioned material blur remains. tsc 0 / 960 tests / lint 0. Operator: eyeball 4 overlays in light+dark. |
| 137 | Design-token compliance batch (chart-6/7 tokens, accent-fg, formatPercent, field labels, QuickAdd chips) | P3 | M | after 134 if both run | ✅ MERGED (4/5 items) on main (fast-forward `ac01fc70`, 2026-07-09). **A** `--ns-chart-6/7` in all 3 theme blocks + DESIGN.md §2.5; raw hex removed across 7 route files. **B** `#fff`/`#000` on accent → `--ns-accent-fg`/`--ns-bg-elev` (4 legit dynamic-color remainders documented). **D** `.ns-field-label` applied to 2 clean sites (others skipped: the class is unlayered so a Tailwind `mb-*` can't override its 6px — pixel-identity couldn't be met). **E** QuickAdd chips → DS tokens + display via `formatNumber`/`formatPrice`. **C SKIPPED (correct STOP): `formatPercent` expects a RATIO (`×100` inside) but every call site holds percent-scale numbers — swapping would 100× the output.** tsc 0 / 960 tests / lint 0. |
| 138 | Accessible ModalShell (role/focus-trap/Escape) + 4 pilot migrations | P2 | L | 136 rec. | ✅ MERGED on main (merge `9f30aacb`, 2026-07-09). New `ModalShell` (role=dialog, aria-modal, focus save/trap/restore, Escape + scrim close, body scroll-lock, center/sheet/drawer variants, no new dep) + **11 a11y tests**. 4 pilots migrated (HoldingEditModal, TransactionDetailPanel, AccountsRoute adjust, CategoryManagementDrawer). **Key: in-modal popovers (AppSelect/DatePicker/IconPicker) portal to body; the trap binds keydown to the PANEL node so it never fights them (unit-tested).** CategoryManagementDrawer uses `disableEscape` (its inline editors own Escape). DESIGN.md §6.4 → ModalShell. tsc 0 / 971 tests / lint 0. **~10 remaining overlays inventoried for per-file follow-up.** Operator: eyeball the 4 pilots (focus/Tab/Escape/in-modal pickers). **Tail update (2026-07-11 reconcile):** PR #16 (`feat/ai-modalshell-tail-b`) landed 2 more (accounts wizard, goal-editor sheet); the wider `feat/ai-modalshell-tail` branch (10 overlays) is UNMERGED and now CONFLICTS with main — redo per-file against current main, do not merge as-is. |
| 139 | Docs reality-sync (architecture/README/ROADMAP/DEVELOPMENT/COSS-plan) + drop 3 unused deps | P2 | S–M | skip §130-overlap if 130 landed | ✅ MERGED on main (fast-forward `7b412151`, 2026-07-09). architecture.md Supabase→Connect Worker (grep 0); README ×2 drop hidden-DCA claim; ROADMAP shipped-items moved + fee/tax-split struck; DEVELOPMENT.md Zod removed (grep 0) + CI-pipeline clarification; coss-migration-plan reality addendum; dropped react-hook-form/@tanstack/react-table/next-themes (build + tsc + 960 tests + lint all green — production build proven without them). Honest: did NOT fabricate an unverifiable "holding-identity shipped" claim; 130's Stronghold doc lines correctly left. Suggested RELEASING.md checklist line to keep ROADMAP honest. |
| 140 | Analytics trust P1s (router error boundary, QueryGate, fixed-basket caveat, tappable help) | P2 | M | — | ⏭️ REJECTED / SUPERSEDED (2026-07-09) — all 4 deliverables already shipped on main independently since the 2026-06-15 audit that seeded this plan: `router.tsx:148` `defaultErrorComponent: RouteError`; DashboardRoute + InvestmentsRoute render Skeleton on `isInitialLoading` + error strip w/ 重新整理 on `isError` (gated on isError not isFetching); AnalyticsTab:539/825 fixed-weight caveats + MetricHelp; MetricHelp uses Base UI Popover (click + keyboard, not hover). Executor verified & STOPPED rather than build a spinner `QueryGate` that would REGRESS the richer skeletons. (Future DRY-only: a shared skeleton-preserving QueryGate for new routes — low priority.) |
| 141 | Custom-asset staleness data-health rule + entry-flow gap check | P2 | S–M | — | ✅ MERGED on main (merge `6140ad14`, 2026-07-09). The `stale-manual-price` rule pre-existed (`b27cb8ae`); this aligned it to spec — threshold 5→90 days (`CUSTOM_PRICE_STALE_DAYS`, manual assets reprice slowly), split into stale + no-snapshot (`已超過 90 天未更新` / `尚未記錄任何價格`) variants, surfaced on the Dashboard banner CTA. Kept `warn` severity (sibling staleness rules are warn; flagged). tsc 0 / 960 tests / lint 0. **Flow-walk gaps CLOSED (2026-07-11 reconcile): PR #15 commit `67064f06` shipped discoverable custom-asset entry + per-holding stale-price badge.** |
| 142 | DCA finish-or-retire decision doc (spike — docs/dca-decision.md) | P3 | S–M | — | TODO |
| 143 | Household sharing (Connect Duo) design spike (docs/household-sharing-spike.md) | P3 | M | read 130–132 | TODO |

Batch dependency notes:
- **144 → 124**: the plan-124 executor's mandatory invalidation audit found
  `AccountsRoute.recalculate` persists recomputed assets but only refetches
  `accounts` (never invalidates `assets`) — reviewer-confirmed against
  GeneralSection's correct sibling. Landing 124's config with this gap open
  would leave holdings stale for the session. 144 fixes it; 124 re-runs after.
- **119 → 126**: the harness would immediately flag 119's two known divergences;
  fix first so its first run is clean.
- **126/127 → 125**: recompute changes are money-critical; land the safety net first.
- **130 → 131 → 132** is a hard security sequence (storage → key exchange →
  credentials). 132's crypto-revocation gap (vault-key rotation) is an explicit
  deferred follow-up recorded in its plan.
- **121 + 123** both touch `createFxConverter` — trivially composable, but
  whichever runs second re-checks the factory excerpt.
- **134 before 137** if both run (shared files: HoldingDetailRoute, AnalyticsTab).
- Deep-audit clean bill (do not re-audit): `npm audit` 0 vulns; AES-GCM envelope
  crypto sound (fresh IVs); no XSS sinks (MarkdownText parses to elements); all
  SQL parameterized; `.env` never committed; entry bundle clean; no circular deps.

## 2026-07-11 GitHub security-alerts batch (150)

Source: operator review of the GitHub Security tab (9 Dependabot + 7 CodeQL open
on `main`). Planned against `main` @ `da946482`. All findings vetted against the
code: the 2 High CodeQL alerts are a real (but low-practical-risk) taint chain
from `deviceIdentity.ts:33`'s `Math.random()` fallback into the sync Bearer
token; the 5 Medium are missing `permissions:` in ci.yml; the 8 npm Dependabot
alerts are dev-scope transitives of the worker's `wrangler` (fixed by lockfile
refresh). **Rejected/deferred**: Rust `glib` 0.18.5 alert — pinned by Tauri 2's
gtk 0.18 Linux stack, no local fix; operator should dismiss or await a Tauri
upgrade. The "CodeQL is reporting errors" banner is scanning infra, not repo
code — operator to check which language/job errors once.

| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 150  | Clear GitHub security alerts — CSPRNG device-ID fallback, CI permissions, worker dep bumps | P2 | S | — | DONE (executed + reviewed 2026-07-11; branch `fix/ai-security-alerts` @ `ebc37851`, unmerged; reviewer re-verified all gates: Math.random grep 0 / ci.yml permissions / ws 8.21.0 + undici 7.28.0 + esbuild 0.28.1 / root tsc 0 + lint 0 + 984 tests / worker tsc 0 + 33 tests; one approved deviation: randomUUID guard uses `typeof crypto.randomUUID === "function"` instead of the plan's `"randomUUID" in crypto` — the `in` form fails tsc narrowing (DOM lib makes the negated branch `never`); runtime semantics identical. Alerts close on next main scan after merge; glib alert stays open by design — operator dismisses) |

## 2026-07-11 operator bug batch (151–155)

Source: `/improve plan` with five operator-reported bugs. Planned against
`main` @ `da946482`. Every root cause was verified before planning — the two
search bugs against live data sources (the reported fund exists in the SITCA
CSV under 受益憑證代號 `T1605Y`, which the app never indexes; Yahoo's search
endpoint empirically returns nothing for 台積電/富邦金), the two layout bugs
reproduced in the dev preview with the mechanism confirmed via DOM inspection
(sidebar z-1100 vs QuickAdd z-80; body scroll-lock inert + sticky-breaking
under `html{overflow-x:clip}`), and the category bug traced to
`normalizeCategoryGroups` provably stripping `kind`/`rollover`/`rolloverStart`
on every save.

All five are independent — any order works. Suggested: 154 first (data loss),
then 155, 151, 153, 152.

| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 151  | SITCA funds findable by 受益憑證代號 (e.g. T1605Y) | P1 | S | — | ✅ MERGED on main (merge `af99c4e0`, 2026-07-11; commit `1a4c9e80`). certCode parsed + matched in filterFunds; 13/13 provider tests incl. the real T1605Y regression row; reviewer re-ran tsc/tests in the worktree. Live end-to-end blocked by the pre-existing dev-proxy 502 (see batch note) — Tauri path unaffected; operator can spot-check in the app. |
| 152  | Ticker search by Chinese name (local TWSE/TPEx name index) | P2 | M | — | ✅ MERGED on main (merge `92bc0bf5`, 2026-07-11; commit `bbf5beb3`). nameShort captured; pure `filterTaiwanSecurities` + `searchSecurities` merged into useSymbolSearch (yahoo → tw-local → funds, deduped); 7 new filter tests. Reviewer verified the UI fires the new TWSE/TPEx/STOCK_DAY_ALL requests on typing 台積電; network data in the browser dev shell blocked by the pre-existing dev-proxy 502 (Tauri unaffected) — operator spot-check in the app. |
| 153  | Quick Add bar clipped by sidebar in narrow windows | P1 | S | — | ✅ MERGED on main (merge `3937373e`, 2026-07-11; commit `981b50b7`). Overlay left-offset by live sidebar width (64/240) with <1024px reset to 0, EntryDrawer-scrim pattern; z-1100 decision untouched. Live-verified post-merge at 1040×650: bar fully visible, sidebar not greyed. |
| 154  | Category 收入/支出 kind (+rollover) stripped on save — persist it | P1 | S–M | — | ✅ MERGED on main (merge `e7928f96`, 2026-07-11; commit `63932c4f`). normalizeCategoryGroups now preserves kind/rollover/rolloverStart (fail-first proven: 6 of 10 round-trip cases failed pre-fix); drawer re-seeds `local` on closed→open. Live-verified post-merge: kind=income persisted in IndexedDB across reload; 支出 picker hides the 收入 category, 收入 picker shows it. One approved deviation: explicit union cast on `kind` (tsc widening). |
| 155  | Scroll-lock on body inert under overflow-x:clip — lock documentElement | P1 | S | — | ✅ MERGED on main (merge `733d7b6a`, 2026-07-11; commit `eb3bdcf8`). Shared `lockViewportScroll()` in src/lib; ModalShell + EntryDrawer switched; `body.style.overflow` grep → 0; 5 helper tests (incl. honest out-of-order-release limitation) + 11 ModalShell tests green. Live-verified post-merge: drawer open ⇒ wheel scroll locked (scrollY stays 0), sidebar sticky intact, lock releases on close. |

Batch notes:
- 154 is the highest-value fix: `normalizeCategoryGroups` silently discards
  `kind` (plan 056's feature), `rollover` and `rolloverStart` (plan 039's
  feature) on EVERY settings save — those features never actually persisted.
  It also fixes the CategoryManagementDrawer's stale mount-time snapshot.
- 155 answers the operator's「Sidebar 是不是要 always on top?」question: the
  sidebar sticky was correct; the drawer's body scroll-lock silently failed
  (root has `overflow-x: clip`, so body overflow never propagates to the
  viewport) AND broke sticky. Locking `documentElement` was verified live.
- 153 deliberately does NOT touch the sidebar's z-1100 decision (plans 052/063);
  it offsets the QuickAdd overlay to the content area, same pattern as the
  EntryDrawer scrim.
- 151+152 both extend ticker search but touch different providers; trivially
  composable. English-name search already works via Yahoo — only Chinese needs
  the local index (152).
- Vetted, not re-flag: `TickerSearchField`'s `.toUpperCase()` on input is
  harmless (CJK has no case; codes want uppercase). QuickAdd having no scroll
  lock is a product call, noted in 153/155, not a bug.
- **Found during post-merge verification (pre-existing, NOT from this batch):**
  the browser dev shell's `/api/market-data` Vite middleware 502s on every
  request — Connect leaves `request.url = "/?url=…"` after mount-path
  stripping, the `replace(/^\?/, "")` doesn't strip `"/?"`, so the `url` param
  reads null → `new URL("")` throws → 502 "Invalid URL". `vite.config.ts` is
  untouched since plan 092 (`a725b014`), so SITCA/TWSE data has never loaded
  in the browser dev shell; the Tauri app uses the Rust `fetch_market_data`
  command and works. One-line fix: parse via
  `new URL(request.url ?? "/", "http://localhost").searchParams`. Flagged as a
  background task chip 2026-07-11; fix in vite.config.ts only.

## Dependency notes

- All current UI phase 3 plans are fully executed, verified, merged to `main`, and released in `v0.1.0-alpha.46`.
- **Apple-platform track (077, 078, 079)**: 078 is a decision doc (no execution) —
  it concluded the project stays on Tauri rather than React Native for mobile. 077
  (iOS) and 079 (macOS) are the two execution plans that follow from that decision;
  they share the same codebase and the same reminder-count selector (077 Phase 6.1
  notifications ↔ 079 Step 3 Dock badge). Either can go first; if 077 Phase 6.1 lands
  first, 079 Step 3 reuses its scheduler.

## 077 phase breakdown (mega-plan executed in slices)

077 is a 7-phase plan, not a single executable unit. Slices carved out as standalone plans:
- **Phase 3.1** (cursor durability) → **plan 081**, but the design was CORRECTED: 077's
  suggested SQLite `_sync_meta` table is REJECTED (it would break the device-locality of
  `deviceId`/cursors, since SQLite is backed up & synced). 081 uses a `plugin-fs` file
  mirror in `AppLocalData` instead.
- **Phase 3.2 throttle** is ALREADY DONE in `useAutoSync` (`MIN_SYNC_INTERVAL_MS`); only
  the iOS `visibilitychange`/`tauri://resumed` listeners remain — deferred (touch
  AppShell, GUI-verify).
- **Phase 7 data path** (category list to on-device model) → **plan 080** (DONE).
- **Phases 1/2 (RWD)** → **plan 083 (DONE)**. A code audit found the app was ALREADY
  largely responsive: wide tables are gated `hidden sm:contents` with mobile card lists,
  dashboard grids use `auto-fit/min(...,100%)` collapse, and `.ns-dash-activity-grid`
  collapses at 900px. The only real gaps were 2 inline grids (Analytics 2-col band,
  Categories 4-col summary) — fixed in 083. The 12-month strip was correctly left dense.
- **Phase 4 (Liquid Glass)** → shipped in 079 (`data-ios-glass` CSS material was folded
  into the macOS polish work). No separate plan needed.
- Phases 6.2/7.2/7.3 are explicitly "do not build" (SwiftUI / product-gated).

**Net: 077's main executable work shipped as slices 079–084.** What remains:

### Outstanding — needs YOU (manual verification only; merges are DONE)
- ~~Merge the stack~~ — **DONE**: 079–084 (and `fix/ai-vite-dev-host`) are all on `main` as of 2026-07-02 (verified this reconcile). No merge action left here.
- **Manual verification** (can't be done headlessly, code is already shipped): 079 macOS GUI (title bar / menu / Dock badge / window restore) + iOS-sim build; 082 notification banner fires; 083/084 the 390px eyeball; **084 nav re-test** (does tapping 投資 navigate or go blank at 379px?).
- **Live per-route 390px QA**: the static audit missed the nav + date-strip bugs the user found live (084). Other routes may have similar live-only issues — worth a pass through each at 390px.

### Outstanding — small code gaps NOT yet executed
- **077 Phase 5 — license audit** (`license-checker`, confirm no GPL/LGPL/AGPL/SSPL). Never run. S.
- **077 Phase 7.5 — power/perf budget** (grep `setInterval`, confirm route code-splitting, chart memoization). Never run. S–M.
- **077 Phase 3.2 — iOS lifecycle sync listeners** (`visibilitychange`/`tauri://resumed`). Deferred (touches AppShell, GUI-verify). S.
- **077 Phase 7.4 — Writing Tools check** (confirm memo fields use native inputs). Trivial verify. XS.

### Deferred by design — decide-then-build (SwiftUI / product-gated)
- **086 Widget** + **087 App Intents** — design pinned in **085**; awaiting your simulator-vs-$99 decision + the Tauri-regeneration spike.
- **077 Phase 7.2** — new on-device AI features (transaction auto-categorization, monthly summary). Product-gated design notes, not built.

## Dependency notes — 2026-07-02 batch (096–099)

- 096 and 099 both touch `src/routes/TransactionsRoute.tsx` (096 hides 複製 on
  opening-lot rows; 099 rewrites the summary-cards data source). Execute 096
  first; 099's drift check explicitly tolerates 096's diff.
- 097 fixes the *visual* half of plan 095's pending "panel visual manual-verify"
  note — the notification panel was being clipped by the sidebar's
  `overflow: hidden` (a live-only bug the headless review couldn't catch).

## Dependency notes — 2026-07-02 critique batch (101–107)

Source: `/impeccable critique` full-project review (snapshot at
`.impeccable/critique/2026-07-02T09-45-15Z__src.md`, score 32/40). Operator
chose: fix all P0–P2 across 數字可信度 / 一致性 / 手機版面, plus DX guardrails.

- **101 → 102 is a hard order**: 102's new eslint rule errors on the exact
  lines 101 rewrites. Everything else is independent.
- 103, 105, 106 all touch `src/routes/DashboardRoute.tsx` in different regions
  (hero/pct logic vs. card titles vs. header actions). Execute in numeric order
  and re-locate by grep, not by line number; each plan's drift check tolerates
  the others' diffs.
- 101 and 105 both touch `src/routes/settings/CategoriesSection.tsx`
  (line 229 money display vs. line 197 header) — same rule: grep, don't trust
  line numbers.
- 103 partially resolves the previously deferred "分析的圖表和功能是不是真的有用"
  item: the one confirmed-harmful instance (short-span XIRR annualization
  displayed unannotated) is now specified. The broader usefulness review stays
  deferred.

## Dependency notes — 2026-07-02 evening batch (108–111, user-reported)

Source: operator's 4-item report (小數位數 / DRIP 驗證 / 通知位置 / sidebar
拖曳與間距), with Wealthfolio screenshots as the reference pattern.

- **⚠️ Operator action first: merge `fix/ai-sidebar-titlebar-notification`
  (plan 097, commit `1d17856f`, still unmerged as of 2026-07-02 evening).**
  It already fixes two of the reported symptoms: sidebar logo crowding the
  traffic lights, and the notification panel being clipped. The user is seeing
  main without it. 110 hard-depends on this merge (Step 0 checks ancestry);
  111 runs either way but the spacing half of the complaint is 097's fix.
- 108 → 109 ordered: same file (`InvestmentsAddSheet.tsx`), and 109's repro
  is amplified by 108's input truncation.
- 106 → 110 ordered (both edit the DashboardRoute header actions row).
- 109 touches the shared DRIP validator in `repositories.ts` used by BOTH the
  in-memory and SQLite repos — it is one function; executor must verify both
  call sites route through it (STOP condition included).

## Deferred from the 2026-07-02 critique (not planned yet)

- ~~Dashboard KPI 色籤與資產配置圓餅對色~~ — **RESOLVED 2026-07-03**: operator
  chose 現金同色 only → **plan 112**.
- ~~年度報表頁定位~~ — **RESOLVED 2026-07-03**: operator chose 報稅深化 (not
  year-in-review) → **plan 114**.
- ~~三套樣式系統並存~~ — **RESOLVED 2026-07-03**: operator chose 手動清舊 →
  **plan 115** (rule + phased cleanup).

## Deferred from the 2026-07-02 user request (not planned yet)

- **"分析的圖表和功能是不是真的有用" (analytics usefulness review)** — a
  product-direction critique of InvestmentsAnalyticsTab / dashboard charts, not
  a specifiable fix. Needs either a dedicated `/improve next` direction audit or
  a live `/impeccable` session with the operator in the loop. The one concrete,
  confirmed instance (summary cards ignoring filters) became plan 099.
- **RecurringRulesTab 分類 is a free-text input** (RecurringRulesTab.tsx:460-467)
  while every other entry surface uses a structured picker — inconsistent UX,
  and it bypasses category-kind tagging entirely. Worth its own small plan when
  recurring rules get attention.

## Findings considered and rejected

- (P3) Dashboard card-heaviness: Flattening cards would require significant visual redesign across DashboardRoute. Better addressed as a dedicated design sprint, not an incremental plan.
- Sidebar width transition in AppShell.tsx: Intentional structural animation, not a data-driven bar. Not a layout thrashing issue.
- `InvestmentsRoute.tsx:1339`/`1448` `hover:bg-black/5 dark:hover:bg-white/5`: impeccable 偵測器 flag 為 pure-black background — 誤報，是合法的列 hover 微調，非 scrim。107 明確排除，勿再掃出。
- Dashboard KPI 卡的 4px 色籤（`KpiCard`, DashboardRoute.tsx:1370）曾疑似 side-stripe 反模式 — 查證為圓角 pill 元素（非 border-left），屬允許寫法，不修。
- QuickAdd FAB 蓋到 Dashboard 圖表右下（375px）— 標準 FAB 行為，demo 資料下才明顯，影響低，不值得做。

### Follow-ups opened this session (not yet planned)

- **137-C — `formatPercent` migration is a mis-fit, WON'T-DO as specced**: `formatPercent(value)` does `value*100` (expects a ratio), but the ~20 call sites hold percent-scale numbers with bespoke +/−/abs sign handling. A real consolidation would need a percent-scale variant + sign-handling audit — a separate, larger plan if desired. Not the drop-in the plan assumed.
- **138 remaining overlays (~10)** for per-file ModalShell migration: AccountsRoute wizard drawer, RecurringRulesTab, RecurringInvestmentsTab, GoalEditorSheet, ReconcileRoute ×2, InvestmentImportWizard, ManualPriceImportWizard, InvestmentsAddSheet, InvestmentsRoute, CashFlowRoute drawer, ConnectSection. (QuickAdd/OnboardingOverlay deliberately excluded.)
- **141 flow-walk gaps**: custom-asset creation buried under the「匯入現有持倉」button; stale-price signal is Dashboard-aggregate only (no per-holding badge).
- **133-C DONE by operator 2026-07-10**: CSP `connect-src` pinned to `https://northstar-sync.larrynote.workers.dev` (commit `061603e4`).
- **132 deferred spike**: vault-key rotation on revocation (a revoked device that captured ciphertext can still decrypt THAT; future data is already cut off by per-device auth). Needs 131's /keys machinery; substantial.

### From the 2026-07-09 deep audit (vetted-rejected — do not re-flag)

- "DESIGN.md prescribes SwiftUI" — mis-attributed by a subagent; the line is in the known-stale `.impeccable.md:17`, not DESIGN.md. DESIGN.md is accurate.
- TransactionsRoute 「JANUARY 2026」English month header (`transactionsTxLabel`-adjacent, ~:643) — intentional: explicit eslint-disable + comment; matches the English-eyebrow convention. Not stray i18n.
- QuickAdd editable-input `toLocaleString` (~:261) and NumberField/FIRECalculator input formatting — inputs are exempt from the privacy-mask rule by convention (you can't mask a field mid-edit). Only DISPLAY chips were planned (137-E).
- Direct `getFinanceRepository()` calls in 10 files — mostly legitimate imperative one-offs (demo mode, export, device connect); not worth a consolidation plan.
- TypeScript 7 / ESLint 10 / worker-types v5 majors — track, don't migrate; all runtime deps are current-major. Batch the ESLint ecosystem when it's actually needed.
- Sortino/Sharpe/MaxDrawdown KpiCard ACCENT colors using pos/neg (AnalyticsTab :843/:861) — metric-quality cues, not price direction; the gain/loss litmus does not apply. Recorded in plan 134 as leave-alone.
- worker CORS `*` — re-confirmed fine (Bearer auth, no cookies). Standing rejection from the June audit.
- "Custom assets have no entry UI" (original DIR-04 wording) — STALE at vetting: InvestmentsAddSheet:304 creates `assetType:"custom"` and HoldingEditModal logs manual prices. Only the staleness data-health rule remained → plan 141 re-scoped.
- `next-themes`/`react-hook-form`/`@tanstack/react-table` as "maybe planned" — no; verified zero imports, removal is plan 139 item 6.
- Dead COSS primitives (coss/checkbox|field|label|select, 0 importers) — intentional scaffolding for the deferred form-primitive migration; plan 139 documents them instead of deleting.
- `components/ui/` "dual component stack" — largely by-design per `src/components/ui/README.md`'s whitelist (command/popover/date-picker have no COSS counterpart); only the migration-plan doc's "COMPLETE" wording drifted → folded into plan 139.
- NotificationCenter/FilterPill/SegmentedControl small `rgba(0,0,0,…)` shadows and CashFlow/QuickAdd active-chip `rgba(0,0,0,0.12)` borders — subtle elevation/edges, not scrims; excluded from plan 136 (leave-alone list inside the plan).
- `InvestmentsRoute.createSnapshot/deleteSnapshot` invalidating only `["manualPriceSnapshots"]` (narrower than HoldingEditModal's siblings) — investigated during plan-124 execution: NOT a gap. Custom-asset valuation re-derives at render from the invalidated `manualPriceSnapshots` query; `createManualPriceSnapshot` only inserts a row, it does not recompute `portfolioAssets`. The siblings' extra keys are defensive redundancy, not required. Do not "fix".
