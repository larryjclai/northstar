...
| 072 | Open-source under MIT | P2 | M | HIGH if rushed | — | TODO — **process plan, operator-gated**. Hard gate: full git-history secrets scan (gitleaks/trufflehog) before any visibility flip — current tree clean + .env/.key never tracked, but history must be audited. Add MIT LICENSE; bank logos (private-assets, trademark) + fonts (OFL) stay separately licensed; dep-license check; README/SECURITY public-ready. NEVER flip repo to public unattended. |
