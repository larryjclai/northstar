...
| 024 | Northstar Metric framework + 底氣 metrics (Coverage Ratio & Runway) | P2 | M–L | MED | — | DONE on branch advisor/024-northstar-metrics (af7ab8bf) — NOT yet merged. `northstarMetrics.ts` (+21 tests: trailing-3mo expense, coverage, runway; settled/!neutral convention) consuming `buildDividendAnalysis.ttmTotal`; `northstarMetric` pref (default netWorth, persists like benchmarkTicker); Dashboard hero registry (netWorth/savingsRate/coverage/runway/FIRE%) with netWorth preserving today's exact hero + a 北極星指標 picker. Reviewed: domain helpers correct + re-run clean, tsc/build/lint(0)/test(513) clean, finance fns not altered. **CAVEAT/follow-up**: runway liquid-cash was *replicated inline* (byte-identical to `calculateAvailableCash`) instead of imported — works today but should be refactored to import the function (DRY / drift-risk, per 019 principle). |
