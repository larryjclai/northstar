...
| 070 | Two-level sector taxonomy (canonical GICS-11 + fine industry), stored in DB | P2 | M | MED | 068, 069 (MERGED) | TODO — sector breakdown mixes TWSE 39-industry (TW) + Yahoo GICS-11 (US/ETF) → incoherent cross-market. Add canonical GICS-11 (`sectorCanonical` col) + keep `industry` fine; TWSE→canonical mapping table (tested); default breakdown canonical, industry as drill-down. Operator: 「很重要」+ wants 2 levels in DB. |
