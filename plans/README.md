...
| 071 | Server-side yfinance ETF sector(+country) pipeline → privacy-preserving client feed | P2 | P0 ~0.5d / P1 ~1.5-2.5d | MED | 070, 068, 069 | TODO — **design+spike-gated**. Live-tested: yfinance `funds_data.sector_weightings` covers US+intl+**TW** ETFs free/no-key (Finnhub free = all 403 premium). Server-side (Python cron → static CDN JSON) → client pulls generic feed (no holdings leak) → lights up 068's dormant weighted split via 070 taxonomy. Country tiered (direct local done; global ETF = 「全球/多國」 bucket v1). Gate: infra + privacy mode sign-off. |
