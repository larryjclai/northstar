...
| 023 | P3 UX backlog (verify-first: dashboard labels, goal pace, FX direction, holding empties, sortable tables, detail export, KPI hierarchy*, FIRE sensitivity*) | P3 | M | LOW | — | A,C,D,F,G DONE+MERGED; B DEFERRED; E BLOCKED; H REJECTED(already built). **A+C** eee260ef, **D** 08f3ae55 (all merged earlier). **F** b7ef4226 (merged 4f11064f): wired the 2 disabled 匯出 buttons on CategoryDetail/MerchantDetail via exportLedgerCsv+downloadCsv. **G** bc4f27b8 (merged): CashFlow 儲蓄率 hero (accent-soft bg) + compact 收入/支出; executor swapped left-border→accent-soft per impeccable hook. **E (sortable tables) BLOCKED**: executor worktree was based on frozen `b0fda83d` (pre-019/020), so its sort edits to CategoriesTab/MerchantsTab conflict with 019/020's rewrites on main — needs re-execution against current main in a fresh session (branch advisor/023-e-sortable-tables kept for reference; do NOT hand-merge). **B (goal pace) DEFERRED**: `FinancialGoal` has no targetDate. **H (FIRE retirement income) REJECTED — already fully built**: `incomeItems` model + `projectRetirementScenarios` math + persistence + a 「退休收入（選填）」editor UI (FIRECalculatorRoute.tsx:364-453) all exist; audit item was stale. **G design decision**: operator chose 儲蓄率 as hero. |
