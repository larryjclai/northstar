...
| 068 | ETF sector + country/region breakdown by holdings (instead of 「未知分類」) | P2 | T0 ~0.5-1d / T1 ~1.5-2.5d | T0 LOW / T1 MED | 067, 069 (both MERGED) | DONE (Tier 0) + MERGED (43d51c78). Tier 0 shipped: direct-holding **country/地區 breakdown** (local ticker/market→ISO map, no fetch) + ETFs get 「ETF/基金」 bucket (never 未知) + manual sector/region tag via 069 lock; Σ=portfolio value per dimension; valuation untouched; 672 tests. **Tier 1 STOPPED at gate** — Yahoo-TW JSON endpoint returns HTTP 400 unattended; 行業比重 only in server-rendered HTML (scraping forbidden per operator). Future: revisit if a real JSON API appears or scraping is later accepted. |
...
| 070 | Two-level sector taxonomy (canonical GICS-11 + fine industry), stored in DB | P2 | M | MED | 068, 069 (MERGED) | DONE + MERGED. canonicalSector.ts (11 GICS keys + zh/en + toCanonicalSector for TWSE codes & Yahoo names, judgment calls documented); sectorCanonical persisted (sector_canonical col, central derive in assetClassificationFields, derive-on-read for legacy); buildSectorBreakdown defaults canonical + level:"industry" drill-down; TW 半導體 + US Technology → one 資訊科技 bucket (tested). 743 tests; valuation untouched; browser≡SQLite. |
| 071 | Server-side yfinance ETF sector(+country) pipeline → privacy-preserving client feed | P2 | P0 ~0.5d / P1 ~1.5-2.5d | MED | 070, 068, 069 | DONE (Phase 1) + MERGED. yfinance weekly GH-Action → Pages feed (canonical-mapped via 070); bundled snapshot (private-assets/etf) + on-demand whole-file pull (no holdings leak); wires AnalyticsPosition.sectorWeights → 068 split (manual>fetched>bucket); bond/global→bucket; tight allowlist (larryjclai.github.io exact path, no query) + vite parity; 755 tests; valuation untouched. **Operator: enable GitHub Pages (Settings→Pages→Source: GitHub Actions).** |
| 072 | Open-source prep under **GPLv3** + CLA | P2 | M | HIGH if rushed | — | DONE + MERGED (74e46ecf). **git-history secret audit CLEAN** (gitleaks 499 commits, 2 false-positives = storage-key NAMES; .env/.key never committed). LICENSE=full GPLv3; package.json GPL-3.0-or-later; CLA.md (Apache ICLA+Harmony, relicense grant) + cla-assistant PR-gate workflow (safe pull_request_target, no PR-code run); THIRD-PARTY (OFL fonts); deps GPLv3-compatible (no AGPL). Docs/config only, 0 src change, 743 tests. **Operator-gated remainder**: fill <COPYRIGHT_HOLDER> in LICENSE+CLA, lawyer-review CLA, ~~CLA-bot PAT+signatures-store (DONE)~~, enable private vuln reporting, then the IRREVERSIBLE public flip. |
...
| 074 | Branch cleanup after going public | P3 | S | MED | — | DONE — swept: 86 merged local + 3 merged remote (feat/*) branches deleted, ~30 stale executor worktrees removed; Group A (advisor/011-017+031 + 4 doc spikes, content on main) + Group B (Tauri/recharts spikes) force-deleted per operator. Kept: main, archive/*, test/cla-check (CLA test PR, pending bot verify). |
| 075 | Contribution norms (PR template, CoC, issue/CONTRIBUTING polish) | P2 | S | LOW | 072 | DONE + MERGED — issue templates already exist; ADD `.github/PULL_REQUEST_TEMPLATE.md` (CLA reminder + real-gates checklist) + `CODE_OF_CONDUCT.md` (Contributor Covenant 2.1, contact flagged) + polish CONTRIBUTING. Docs-only. |
