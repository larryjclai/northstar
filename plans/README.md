...
| 149 | Device reset wipes the SecretStore + account.ts uses the shared store (130 follow-ups) | P1 | S | 130 (merged) | TODO — **security**: post-130, `reset.ts` clears only localStorage, so a device reset leaves vault key/keypair/account inside Stronghold (reset ineffective on device). Also account.ts opens a 2nd store instance. Ship WITH 130 in any release. |
