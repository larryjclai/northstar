...
| 066 | Taiwan mutual-fund NAV via SITCA (auto-price funds not on Yahoo) | P2 | M–L | MED–HIGH | 038/045 (merged) | TODO — **spike-first/design-gated**. Provider abstraction (`MarketDataProvider`) + native `fetch_market_data` allowlist already exist; listed ETFs (0050.TW) already price via Yahoo. Gating unknown: a stable, allowlistable SITCA NAV source (no official API). Phase 0 spike → if none, fall back to manual-price (045)/CSV (048). Operator-requested (Percento parity). |
