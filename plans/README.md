...
| 027 | "Join the index" nudge — design + decision gate | P3 | S–M | MED | — | DONE on branch advisor/027-index-nudge-lite (e4130f2c) — NOT yet merged. Operator chose **Option L (lightweight/neutral)**: plain-language 「期間累積落後/領先 {0050} X%」caption in InvestmentsAnalyticsTab (reuses existing `perf.alpha`) + a `benchmarkGap` north-star metric option in the 024 registry (reuses `stripData.alpha`). No detection/advice/dismiss (not Option A). Reviewed: no new finance calc, portfolioAnalytics untouched, tsc/lint(0)/test(513) clean. **CAVEAT before merge**: the registry entry uses `METRIC_REGISTRY.push(...)` (correct now since the array is a per-render const, but a foot-gun if the registry is ever useMemo'd — should be refactored to declare the array after `stripData`). |
