...
| 067 | Taiwan stocks show English names — re-qualify them for the TWSE Chinese-name backfill | P2 | S–M | LOW–MED | 066 (shared useMarketRefresh) | TODO — root cause: `backfillAssetProfiles` only re-fetches a TW ticker when `nameZh` is MISSING, but Yahoo's chart saves the English name as `nameZh` → never re-qualifies for the real TWSE Chinese name. Fix: re-qualify when no REAL Chinese name (CJK check). Display already prefers a real nameZh. **Execute AFTER 066 merges** (both touch useMarketRefresh). Operator-reported. |
| 068 | ETF sector breakdown by holdings (instead of 「未知分類」) | P2 | M–L | MED–HIGH | 069, 067 (shared file) | TODO — **spike-first**. ETFs get `sector:null` (search payload has none; Yahoo quoteSummary topHoldings crumb-blocked) → 未知. Need an ETF sector-weightings source (TWSE/TPEx PCF + per-holding industry, or Yahoo topHoldings, or fallback to 「ETF」 bucket/manual tag). Analytics: split ETF value across sectors by weight. Operator-requested. |
