...
That grant covers the code in this repository only. Bundled
...
Version 1.1 (OFL-1.1)** — a permissive license that is **separate from** the MIT License
that covers Northstar's own source code:
...
- **`lightningcss`** (and its platform binary) — **MPL-2.0**. Mozilla Public License 2.0 is a
  ...
  toolchain dependency and imposes no obligation conflicting with Northstar's own MIT-licensed
...
- The `northstar` entry reported by license tooling refers to this project itself and is
  declared **MIT** via `package.json`.

No AGPL dependencies are present in the production tree. Third-party components retain their
own license terms when distributed with Northstar.
