    ...
    pub(crate) async fn connect<R: Runtime>(
            ...
            .0
        {
            ...
            "sqlite" => {
                ...
                // Northstar patch: pin the SQLite pool to a single connection.
                // plugin-sql issues each execute()/select() as a separate pooled
                // command and releases the connection (asynchronously) afterwards.
                // A multi-statement transaction (BEGIN / writes / COMMIT spread
                // across separate commands) is only coherent if every statement
                // lands on the same connection. With the default 10-connection
                // pool, the async release window lets the next acquire() open a
                // SECOND connection — wider on slow iOS storage — so COMMIT can
                // run on a different connection than BEGIN ("cannot commit - no
                // transaction is active", code 1) and strand the first connection
                // holding the WAL write lock ("database is locked", code 5).
                // max_connections(1) makes acquire() wait for the one connection
                // instead of opening another, guaranteeing transactions stay whole.
                Ok(Self::Sqlite(
