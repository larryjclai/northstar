...
describeEachRepo("recurring", (makeRepo, repoLabel) => {
  describe("postDueRecurringTransactions", () => {
    ...
    it("advances nextRunDate past today even when every due occurrence already exists", async () => {
      ...
      const preexisting = ["2026-03-05", "2026-04-05", "2026-05-05"].map((d) =>
        occurrenceRow(`rec_rent:${d}`, d),
      );
    ...
    it("posts nothing when the rule is already in the future", async () => {
      const repo = await makeRepo({
        accounts: [account],
        recurringTransactions: [recurring({ nextRunDate: "2099-01-05" })],
      });
      const posted = await repo.postDueRecurringTransactions("2026-05-29");
      expect(posted).toBe(0);
    ...
    it("ignores inactive rules", async () => {
      const repo = await makeRepo({
        accounts: [account],
        recurringTransactions: [recurring({ isActive: false })],
      });
      const posted = await repo.postDueRecurringTransactions("2026-05-29");
      expect(posted).toBe(0);
    });
  ...
  describe("applyRecurringScopeEdit", () => {
    ...
    it("scope=all rewrites every occurrence but keeps each original date", async () => {
      ...
      await repo.applyRecurringScopeEdit(target.id, "all", {
        ...draftFrom(target),
      ...
      expect(after.map((r) => r.date).sort()).toEqual(
        [dates[0], dates[2], "2026-09-09T09:00"].sort(),
      );
      const [rule] = await repo.listRecurringTransactions();
    ...
    it("scope=all is atomic: a mid-loop sibling failure rolls back the whole series (sqlite)", async () => {
      ...
      let calls = 0;
      (
        repo as { updateLedgerTransaction: FinanceRepository["updateLedgerTransaction"] }
      ).updateLedgerTransaction = async (idArg, inputArg) => {
        calls += 1;
      ...
      await expect(
        repo.applyRecurringScopeEdit(target.id, "all", {
          ...draftFrom(target),
          date: "2026-09-09T09:00",
        }),
