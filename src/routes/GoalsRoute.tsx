...
export function GoalsRoute() {
  ...
  const projection = useMemo(
    () =>
      selectedGoal && isFire
        ? projectRetirement({
            goal: { ...selectedGoal, expectedAnnualReturn: activeRate },
            currentValue,
          })
        : null,
  ...
  const chartData = useMemo(() => {
    ...
    if (isFire && projection)
      return projection.series.map((row) => ({ x: row.age, portfolio: row.endBalance }));
  ...
  const chartColor = !isFire
    ? "var(--ns-accent)"
    : activeProjection === "bear"
      ? "var(--ns-chart-3)"
      : activeProjection === "bull"
        ? "var(--ns-accent)"
        : "var(--ns-pos)";
        ...
        {selectedGoal && stats ? (
              ...
              <div>
                ...
                {goals.length > 1 ? (
                      ...
                      return (
                        ...
                        >
                          {g.kind === "fire" ? (
                            ...
                            <Target size={14} color="var(--ns-accent)" />
                          )}
                          {g.name}
                        </button>
                      );
                  ...
                  <div
                    ...
                    style={{ color: "var(--ns-fg-muted)", letterSpacing: 0.5 }}
                  >
                    {selectedGoal.name}
                  </div>
                )}
                <div className="flex items-baseline gap-2 mb-4">
                  {isFire ? (
                    <>
                      <span className="text-[40px] font-semibold" style={{ letterSpacing: -1 }}>
                        {formatCompactMoney(currentValue, selectedGoal.currency)}
                      </span>
                      <span className="text-sm" style={{ color: "var(--ns-fg-muted)" }}>
                        / {formatCompactNumber(stats.target)}
                      </span>
                    </>
                  ) : (
                    <>
                      <span className="text-[40px] font-semibold" style={{ letterSpacing: -1 }}>
                        {selectedGoal.currency} {formatNumber(currentValue)}
                      </span>
                      <span className="text-sm" style={{ color: "var(--ns-fg-muted)" }}>
                ...
                >
                  <div
                    ...
                    style={{
                      background: "linear-gradient(90deg, var(--ns-accent), var(--ns-pos))",
                      transform: `scaleX(${Math.min(1, stats.progress / 100)})`,
                    }}
                  />
                </div>
                <div className="text-body" style={{ color: "var(--ns-fg-dim)" }}>
                  {!isFire &&
                  Object.values(selectedGoal.accountShareMap ?? {}).filter((w) => w > 0).length ===
                    0 ? (
                    <>
                      尚未綁定帳戶 —{" "}
                      <button
                        onClick={() => setEditor({ goal: selectedGoal })}
                        style={{
                          color: "var(--ns-accent)",
                          cursor: "pointer",
                          background: "none",
                    ...
                    >
                      <span>
                        ...
                        {stats.years !== null
                          ? `預估 ${stats.years} 年後達成 (${new Date().getFullYear() + stats.years})`
                          : "尚無法預估"}
                      </span>
                      {(() => {
                        ...
                        const chipStyle: CSSProperties = {
                          ...
                          background:
                            pace.status === "ahead"
                              ? "color-mix(in srgb, var(--ns-pos) 15%, transparent)"
                              : pace.status === "behind"
                                ? "color-mix(in srgb, var(--ns-neg) 15%, transparent)"
              ...
              >
                {isFire ? (
                  ...
                  <>
                    <Stat
                      label="已存金額"
                      value={`${selectedGoal.currency} ${formatNumber(currentValue)}`}
                    />
                    <Stat
                      label="目標金額"
                      value={`${selectedGoal.currency} ${formatNumber(stats.target)}`}
                    />
                    <Stat
                      label="年儲蓄"
                      value={`${selectedGoal.currency} ${formatNumber(stats.annualSaving)}`}
                    />
            ...
            <div className="flex-1 flex flex-col" style={{ minWidth: 280 }}>
                    ...
                    {(
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: "1px solid",
                          background:
                            activeProjection === k ? "var(--ns-surface-strong)" : "transparent",
              ...
              <div className="flex-1 w-full" style={{ minHeight: 220 }}>
                {chartData.length > 1 ? (
                  ...
                  <div
                    className="muted text-body flex items-center justify-center"
                    style={{ height: "100%" }}
                  >
                    補上年齡 / 報酬假設後即可預估退休曲線。
                  </div>
          ...
          <Card className="text-center" style={{ padding: 40 }}>
            ...
            <div className="muted text-body" style={{ marginBottom: 18 }}>
              用 FIRE 計算機追蹤財務自由進度，或建立旅遊、買車等自訂儲蓄目標。
            </div>
            <div className="flex justify-center" style={{ gap: 10 }}>
              ...
              <Button variant="outline" onClick={() => setEditor({ goal: null })}>
          ...
          {goals.length === 0 ? (
            ...
            <div className="flex flex-col">
              {goals.map((goal, i) => {
                ...
                const boundCount = Object.values(goal.accountShareMap ?? {}).filter(
                  (w) => w > 0,
                ).length;
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      rowGap: 12,
                      padding: "14px 18px",
                      borderBottom: i < goals.length - 1 ? "1px solid var(--ns-border)" : "none",
                    }}
                  >
                    <div
                      className="flex items-center justify-center mr-4 shrink-0"
                      style={{
                        width: 40,
                        height: 40,
                        borderRadius: 10,
                    ...
                    >
                      <Icon
                        size={20}
                        color={color}
                        weight={goal.kind === "fire" ? "fill" : "regular"}
                      />
                    </div>
                    <div className="min-w-0" style={{ flex: "1 1 140px" }}>
                      ...
                      <div className="text-xs" style={{ color: "var(--ns-fg-muted)" }}>
                        {goal.kind === "fire"
                          ? "FIRE · 依淨值估算"
                          : boundCount > 0
                            ? `自訂目標 · 綁定 ${boundCount} 個帳戶`
                            : "自訂目標 · 尚未綁定帳戶"}
                      </div>
                    </div>
                    <div className="text-right" style={{ flex: "0 0 auto" }}>
                      <div className="text-sm font-medium mb-1" style={{ whiteSpace: "nowrap" }}>
                        {goal.currency} {formatNumber(current)}
                      </div>
                      <div
                        className="text-xs"
                        style={{ color: "var(--ns-fg-dim)", whiteSpace: "nowrap" }}
                      >
                        / {formatNumber(target)}
                      </div>
                    </div>
                    <div className="flex items-center gap-3" style={{ flex: "1 1 180px" }}>
                      ...
                      >
                        <div
                          ...
                          style={{
                            background: color,
                            transform: `scaleX(${Math.min(1, progress / 100)})`,
                          }}
                        />
                      </div>
                      <div
                        className="text-body text-right shrink-0"
                        style={{ color: "var(--ns-fg-dim)", width: 48 }}
                      >
                        {progress.toFixed(1)}%
                      </div>
                    </div>
                    ...
                    >
                      {achieved ? (
                        ...
                        >
                          <CheckCircle size={14} weight="fill" /> 達成
                        </span>
                      ) : (
                        <span className="text-body" style={{ color: "var(--ns-fg-dim)" }}>
                          追蹤中
                        </span>
                      )}
                        ...
                        {confirmDeleteId === goal.id ? (
                          ...
                          <>
                            <Button
                              ...
                              onClick={() =>
                                goal.kind === "fire"
                                  ? navigate({ to: "/goals/fire", search: { id: goal.id } })
                                  : setEditor({ goal })
                              }
                            >
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="text-xs mb-1" style={{ color: "var(--ns-fg-muted)" }}>
        {label}
      </div>
