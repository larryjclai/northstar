...
export function CategoriesTab({
  ...
  const uncategorizedPct =
    totalPeriodSpend > 0 ? (uncategorizedAmount / totalPeriodSpend) * 100 : 0;
  const uncategorizedTopMerchantRaw = topMerchantFor("");
  const uncategorizedTopMerchant =
    uncategorizedTopMerchantRaw === "無" ? "−" : uncategorizedTopMerchantRaw;
      ...
      >
        <Card className="py-5 px-6">
          ...
          <div className="text-lg font-medium">
            {maxSpendCat
              ? `${maxSpendCat.name} · ${primaryCurrency} ${formatNumber(maxSpendCat.amount)}`
              : "無"}
          </div>
        </Card>
        <Card className="py-5 px-6">
        ...
        <Card className="p-6">
          ...
          >
            <div>
              ...
              <div className="num text-[24px] font-medium">
                {primaryCurrency} {formatNumber(totalPeriodSpend)}
              </div>
            </div>
            <Button variant="ghost" size="icon-sm" aria-label="分類設定" onClick={onSettingsClick}>
              <Gear size={16} />
            </Button>
          </div>
              ...
              <div key={r.name} className="text-body flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                    <Glyph name={r.icon} size={14} /> {r.name}
                  </span>
                </div>
                <span className="num muted">
                  {(totalPeriodSpend > 0 ? (r.amount / totalPeriodSpend) * 100 : 0).toFixed(1)}%
                </span>
              </div>
            ))}
        ...
        >
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="muted truncate text-xs">
                      {r.count} 筆 · {pct.toFixed(1)}%{r.topMerchant ? ` · ${r.topMerchant}` : ""}
                    </div>
                  </div>
                  <div className="num text-sm" style={{ whiteSpace: "nowrap" }}>
                    −{primaryCurrency} {formatNumber(r.periodAmount)}
                  </div>
                </Link>
              );
            })}
          ...
          <div className="hidden sm:contents">
            ...
            >
              ...
              >
                分類{categorySort.key === "name" ? (categorySort.dir === "asc" ? " ▲" : " ▼") : ""}
              </button>
              <button
                style={{
                  background: "none",
              ...
              >
                筆數{categorySort.key === "count" ? (categorySort.dir === "asc" ? " ▲" : " ▼") : ""}
              </button>
              <button
                style={{
                  background: "none",
                ...
                onClick={() => toggleCategorySort("amount")}
              >
                期間支出
                {categorySort.key === "amount" ? (categorySort.dir === "asc" ? " ▲" : " ▼") : ""}
              </button>
              <div>佔比 / 主要商家</div>
              <div></div>
            </div>
            <div className="flex-1" style={{ overflowY: "auto" }}>
              ...
              {uncategorizedAmount > 0 && (
                ...
                >
                  ...
                  <div className="num">
                    −{primaryCurrency} {formatNumber(uncategorizedAmount)}
                  </div>
                  <div className="flex items-center gap-2">
                    <span
                      className="text-caption"
