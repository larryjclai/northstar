...
export function CategoriesRoute() {
  const { ledger, settings, dailyFxRates, isInitialLoading, isError, error, refetchAll } =
    useFinanceData();
  ...
  const [catRouteSort, setCatRouteSort] = useState<{
    key: "name" | "amount" | "usage";
    dir: "asc" | "desc";
  }>({ key: "amount", dir: "desc" });
  ...
  const allExpenseRows = filteredRows.filter(
    (row) => row.entryType === "expense" && !row.counterAccountId,
  );
  ...
  const categoryStats = useMemo(() => {
    return periodSpend.categories.map((cat, index) => {
      ...
      if (rollover && budget !== null) {
        const series =
          monthlySpendByCategory.get(cat.name) ?? new Array(trailingMonths.length).fill(0);
        let startIndex = 0;
        if (catSetting?.rolloverStart) {
          const idx = trailingMonths.indexOf(catSetting.rolloverStart);
          if (idx >= 0) startIndex = idx;
        }
        const result = computeRolloverSeries({
  ...
  return (
    <div style={{ padding: "32px 40px 100px", overflowY: "auto", minHeight: "100vh" }}>
      ...
      <div className="flex justify-between" style={{ alignItems: "flex-end", marginBottom: 32 }}>
        <div>
          <div
            className="text-caption muted mb-2"
            style={{ fontFamily: "var(--ns-font-mono)", letterSpacing: 1.5 }}
          >
      ...
      <div className="ns-cat-summary-grid">
        ...
        <SummaryCard
          label="超支分類"
          value={
            overSpentCats.length > 0 ? `${overSpentCats.length} (${overSpentCats[0].name})` : "0"
          }
        />
      </div>
      {missingFxPairs.length ? (
        <div className="ns-surface text-body mb-4" style={{ padding: "10px 14px" }}>
          總額不完整：缺少 {missingFxPairs.join("、")} 匯率。
        </div>
      ) : null}
      ...
      <div className="flex gap-6">
              ...
              <div className="w-full mb-6" style={{ height: 260, position: "relative" }}>
                <ResponsiveContainer>
                  <PieChart>
                    ...
                    <Tooltip
                      formatter={(value) => [
                        `${formatNumber(Number(value))} ${primaryCurrency}`,
                        "金額",
                      ]}
                      contentStyle={{
                        borderRadius: 8,
                ...
                {categoryStats.map((cat) => (
                  <div
                    ...
                    style={{
                      ...
                      background:
                        selectedCategory === cat.name ? "var(--ns-bg-hover)" : "transparent",
        ...
        <Card className="flex-1" style={{ padding: "24px 0" }}>
          {/* List Header */}
          <div
            className="text-caption muted flex"
            style={{
              padding: "0 32px 12px",
          ...
          >
            <button
              ...
              onClick={() => toggleCatRouteSort("name")}
            >
              分類{catRouteSort.key === "name" ? (catRouteSort.dir === "asc" ? " ▲" : " ▼") : ""}
            </button>
            <button
              ...
              onClick={() => toggleCatRouteSort("amount")}
            >
              已消費
              {catRouteSort.key === "amount" ? (catRouteSort.dir === "asc" ? " ▲" : " ▼") : ""}
            </button>
            <div className="text-right" style={{ flex: "0 0 240px" }}>
              預算
            </div>
            <button
              ...
              onClick={() => toggleCatRouteSort("usage")}
            >
              使用率{catRouteSort.key === "usage" ? (catRouteSort.dir === "asc" ? " ▲" : " ▼") : ""}
            </button>
          ...
          <div className="flex flex-col">
                ...
                <div key={cat.name}>
                  <div
                    ...
                    style={{
                      padding: "16px 32px",
                      borderBottom:
                        index < categoryStats.length - 1 ? "1px solid var(--ns-border)" : "none",
                    ...
                    <div className="flex items-center gap-4" style={{ flex: "0 0 160px" }}>
                      <div
                        ...
                        style={{
                          ...
                          height: 36,
                          borderRadius: 10,
                          background: (cat.color || "var(--ns-surface-strong)") + "18",
                      ...
                      <div>
                        <div className="text-[15px] font-medium" style={{ marginBottom: 2 }}>
                          {cat.name}
                        </div>
                    ...
                    <div style={{ flex: "0 0 240px", paddingLeft: 48 }}>
                      ...
                      <div className="text-caption muted">
                        {hasBudget ? (
                          isOver ? (
                            <span className="neg">
                              {percent.toFixed(0)}% · 超支{" "}
                              {formatMoney(cat.amount - cat.budget!, primaryCurrency)}
                            </span>
                          ) : (
                      ...
                      {cat.rollover && cat.carry !== 0 && (
                        <div
                          ...
                          style={{
                            marginTop: 2,
                            color: cat.carry > 0 ? "var(--ns-pos)" : "var(--ns-neg)",
                          }}
                        >
                          {cat.carry > 0 ? "結轉 +" : "結轉 "}
                          {formatMoney(cat.carry, primaryCurrency)} · 可用{" "}
                          {formatMoney(cat.available, primaryCurrency)}
                        </div>
                      )}
                    </div>
                    ...
                    >
                      <button
                        ...
                        style={{
                          ...
                          cursor: "pointer",
                          color:
                            annualCategory === cat.name ? "var(--ns-fg)" : "var(--ns-fg-muted)",
                          fontFamily: "var(--ns-font-mono)",
                        }}
                        title="年度檢視"
                        onClick={(e) => {
                          e.stopPropagation();
                          setAnnualCategory((prev) => (prev === cat.name ? null : cat.name));
                        }}
                      >
                        年
                      </button>
                      <Gear
                        size={16}
                        style={{ cursor: "pointer" }}
                        className="hover:text-[var(--ns-fg)] transition-colors"
                        onClick={(e) => {
                          e.stopPropagation();
                  ...
                  {annualCategory === cat.name && (
                    <AnnualGrid
                      ...
                      series={
                        monthlySpendByCategory.get(cat.name) ??
                        new Array(trailingMonths.length).fill(0)
                      }
            ...
            {categoryStats.length === 0 && (
              <div className="muted text-center" style={{ padding: "40px 0" }}>
                沒有分類資料
              </div>
            )}
          </div>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ fontFamily: "var(--ns-font-mono)", letterSpacing: 1 }}
      >
        <span>近 12 個月</span>
        <span>
          年度支出 {formatMoney(summary.annualSpend, primaryCurrency)} / 年度預算{" "}
          {formatMoney(summary.annualBudget, primaryCurrency)}（月預算 ×12）
        </span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: 6 }}>
        {summary.months.map((m, i) => {
          ...
          return (
            <div
              key={months[i]}
              title={`${months[i]} · 支出 ${formatMoney(m.spend, primaryCurrency)}${rollover ? ` · 可用 ${formatMoney(m.available, primaryCurrency)}` : ""}`}
            ...
            >
              ...
              >
                <div
                  style={{
                    height: `${pct}%`,
                    background: over ? "var(--ns-neg)" : "var(--ns-accent)",
                  }}
                />
              </div>
              <div
                className="text-micro muted text-center"
