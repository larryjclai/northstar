...
export function ReconcileRoute() {
  ...
  const { accounts, ledger, creditGroups, isInitialLoading, isError, error, refetchAll } =
    useFinanceData();
  ...
  const setReviewed = useRepositoryMutation(
    (repository, input: { id: string; reviewed: boolean }) =>
      repository.setLedgerReviewed(input.id, input.reviewed),
    ["ledger"],
  );
  const setPostDate = useRepositoryMutation(
    (repository, input: { id: string; postDate: string | null }) =>
      repository.setLedgerPostDate(input.id, input.postDate),
    ["ledger"],
  );
  ...
  const groupAccounts = useMemo(() => {
    ...
    const siblings = all.filter(
      (a) =>
        a.deletedAt === null && a.type === "credit" && a.creditGroupId === account.creditGroupId,
    );
  ...
  const rows = useMemo(
    () =>
      (ledger.data ?? [])
        .filter(
          (row) =>
            groupAccountIds.has(row.accountId) &&
            row.deletedAt === null &&
    ...
    try {
      for (const a of groupAccounts) {
        await updateAccount.mutateAsync({
          ...
          creditLimitGroup: a.creditLimitGroup,
  ...
  if (!account) {
    return (
      <div style={{ padding: "24px 32px" }} className="muted">
        找不到帳戶。
      </div>
    );
  }

  const creditGroup = account?.creditGroupId
    ? ((creditGroups.data ?? []).find((g) => g.id === account.creditGroupId) ?? null)
    : null;
  const title = isGrouped
    ? creditGroup?.name || account.creditLimitGroup?.trim() || account.name
    : account.name;
  ...
  const currentUnreconciled = (currentPeriod?.rows ?? [])
  ...
  return (
    <div style={{ height: "100%", overflow: "auto", padding: "24px 32px 100px" }}>
      ...
      >
        <span style={{ cursor: "pointer" }} onClick={() => navigate({ to: "/accounts" })}>
          帳戶
        </span>
        <CaretRight size={13} />
        <span className="font-medium" style={{ color: "var(--ns-fg)" }}>
          {title} · 對帳
        </span>
      </div>
      ...
      <div className="flex items-end justify-between flex-wrap gap-4 mb-5">
        <div>
          ...
          <h1
            className="text-[26px] m-0 font-semibold"
            style={{ fontFamily: "var(--ns-font-display)" }}
          >
            {title} 對帳
          </h1>
        ...
        <div className="flex gap-2">
          {account.type === "credit" && account.paymentDueDay && (
            <Button
              ...
              title={
                isPaid
                  ? `已繳款至 ${account.creditPaymentPaidUntil}`
                  : "從帳戶轉帳繳款，可選填帳單折抵 / 回饋"
              }
            >
      ...
      >
        <Card className="p-4">
          ...
          <div
            ...
            style={{ color: currentSpend > 0 ? "var(--ns-neg)" : undefined }}
          >
            NT${formatNumber(currentSpend)}
          </div>
          {currentRefunds > 0.5 ? (
        ...
        <Card className="p-4">
          ...
          <div className="num text-[19px]">
            {currentReconciled} / {currentCount}
          </div>
        </Card>
        <Card className="p-4">
          <div className="text-xs mb-2 font-medium" style={{ color: "var(--ns-fg-muted)" }}>
          ...
          <div
            className="num text-[19px]"
            style={{ color: owed > 0 ? "var(--ns-neg)" : undefined }}
          >
            NT${formatNumber(owed)}
          </div>
        </Card>
      </div>
      {currentUnreconciled > 0 ? (
        <div className="muted text-xs mb-3">
          本期尚有 NT${formatNumber(currentUnreconciled)} 未對帳。
        </div>
      ) : null}
      ...
      {periods.length === 0 ? (
        <Card style={{ padding: "var(--ns-pad-card)" }}>
          <div className="muted text-body text-center" style={{ padding: 40 }}>
            此帳戶尚無交易紀錄。
          </div>
        </Card>
      ) : (
              ...
              <Card key={period.key} className="p-0">
                <div
                  ...
                  style={{
                    padding: "14px 18px",
                    cursor: "pointer",
                    borderBottom: open ? "1px solid var(--ns-border)" : "none",
                  }}
                >
                  <CaretRight
                    size={14}
                  ...
                  <div className="text-right">
                    <div
                      ...
                      style={{ color: period.spend > 0 ? "var(--ns-neg)" : "var(--ns-fg-dim)" }}
                    >
                      NT${formatNumber(period.spend)}
                    </div>
                    {period.spend + period.total > 0.5 ? (
                      <div className="muted text-micro" style={{ marginTop: 1 }}>
                        淨額 NT${formatNumber(-period.total)}
                      </div>
                    ) : null}
                  </div>
                  {open && unreconciled > 0 ? (
                ...
                {open ? (
                  period.rows.length === 0 ? (
                    <div className="muted text-body ns-expand-in" style={{ padding: "16px 18px" }}>
                      本期尚無交易。
                    </div>
                  ) : (
                      ...
                      >
                        {row.isReviewed ? (
                          ...
                          <Circle
                            size={20}
                            className="shrink-0"
                            style={{ color: "var(--ns-fg-dim)" }}
                          />
                        )}
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium truncate">
                            {row.merchant || row.name || row.category || "交易"}
                          </div>
                          <div className="muted text-caption">
                            ...
                            {row.postDate ? (
                              <Badge
                                variant="outline"
                                className="rounded-full text-micro ml-1.5"
                                style={{
                                  padding: "1px 6px",
                                  color: "var(--ns-accent)",
                        ...
                        <div
                          ...
                          style={{
                            color: row.amount < 0 ? "var(--ns-neg)" : "var(--ns-pos)",
                            whiteSpace: "nowrap",
                          }}
                        >
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            ...
                            navigate({
                              to: "/cash-flow",
                              search: { account: accountId, tx: row.id, from: "reconcile" },
                            });
                          }}
                        >
...
function PayCardModal({
  ...
  const [payAmount, setPayAmount] = useState(
    String(selected ? Math.max(0, -selected.total) : owed),
  );
      ...
      {(dismiss) => (
        <>
          ...
          <div className="text-xs mb-4" style={{ color: "var(--ns-fg-muted)", lineHeight: 1.6 }}>
            ...
            {groupCardCount
              ? ` 此組共 ${groupCardCount} 張卡，繳款標記將套用整組；轉帳金額入本卡。`
              : ""}
          </div>
          ...
          <div className="flex justify-end gap-2" style={{ marginTop: 18 }}>
            ...
            <Button
              onClick={() => onConfirm(payAccountId, pay, credit, selected!.dueDate!)}
              disabled={!canConfirm}
            >
      ...
      {(dismiss) => (
        <>
          ...
          <div className="flex justify-end gap-2 flex-wrap" style={{ marginTop: 18 }}>
            ...
            {hasDefer ? (
              <Button variant="outline" onClick={() => onConfirm(null)} disabled={pending}>
                改回當下入帳
              </Button>
            ) : null}
