...
function buildFxStats(
  rates: DailyFxRate[],
): Map<string, { count: number; firstDate: string; lastDate: string }> {
...
export function SettingsFX({
  form,
  submit,
  dailyFxRates,
  t,
}: Omit<SettingsTabProps, "setForm"> & { dailyFxRates: DailyFxRate[] }) {
  ...
  async function refreshAll() {
    if (useDemoMode.getState().active) {
      toast.info("示範模式使用內建行情", {
        description: "已略過線上匯率更新；結束示範模式後即可正常更新。",
      });
      return;
    }
    const pairs = form.exchangeRates.map((r) => ({
      from: r.from,
      to: r.to || form.primaryCurrency,
    }));
  ...
  function addRate() {
    const nextForm = {
      ...
      exchangeRates: [
        ...form.exchangeRates,
        { from: "USD", to: form.primaryCurrency, rate: 1, updatedAt: new Date().toISOString() },
      ],
    };
    submit(nextForm);
  }
  ...
  function updateRate(index: number, val: Partial<ExchangeRate>) {
    const nextForm = {
      ...form,
      exchangeRates: form.exchangeRates.map((r, i: number) =>
        i === index ? { ...r, ...val, updatedAt: new Date().toISOString() } : r,
      ),
    };
    submit(nextForm);
  }
  ...
  function deleteRate(index: number) {
    const nextForm = {
      ...form,
      exchangeRates: form.exchangeRates.filter((_, i: number) => i !== index),
    };
    submit(nextForm);
  }
    ...
    <div className="max-w-4xl">
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Button onClick={refreshAll} disabled={refreshFxRates.isPending}>
      ...
      <Card style={{ padding: 18, marginBottom: 16 }}>
        <div
          className="text-xs"
          style={{ marginBottom: 8, color: "var(--ns-fg-muted)", fontWeight: 500 }}
        >
        ...
        <input
          className="ns-input max-w-xs"
          value={form.primaryCurrency}
          onChange={(e) => submit({ ...form, primaryCurrency: e.target.value.toUpperCase() })}
        />
      </Card>
      ...
      <Card style={{ padding: 0 }}>
        <div
          style={{
            ...
            display: "grid",
            gridTemplateColumns: "80px 1fr 1fr 1fr 56px",
            fontSize: 10.5,
            color: "var(--ns-fg-dim)",
          ...
          return (
            ...
            >
              <input
                className="ns-input"
                value={r.from}
                onChange={(e) => updateRate(i, { from: e.target.value.toUpperCase() })}
              />
              <input
                className="ns-input"
                type="number"
                ...
                value={parseFloat(r.rate.toFixed(6))}
                onChange={(e) => updateRate(i, { rate: Math.round(+e.target.value * 1e6) / 1e6 })}
              />
              <input
                className="ns-input"
                style={{ textAlign: "right" }}
                value={r.to || form.primaryCurrency}
                onChange={(e) => updateRate(i, { to: e.target.value.toUpperCase() })}
              />
              <div className="dim" style={{ fontSize: 11, textAlign: "right" }}>
                {stat ? `${stat.count} records` : "No history"}
              </div>
