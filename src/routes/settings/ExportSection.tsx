...
export function SettingsExport({ t }: Pick<SettingsTabProps, "t">) {
    ...
    () =>
      ledgerRows.filter(
        (row) =>
          !row.deletedAt &&
          inRange(row.date) &&
          accountInScope(row.accountId) &&
          (includeTransfers || row.entryType !== "transfer"),
      ),
  ...
  const estimatedCount =
    filteredLedger.length + (includeInvestments ? filteredInvestments.length : 0);
  ...
  async function runExport() {
    ...
    try {
      ...
      if (filteredLedger.length) {
        ...
        downloadCsv(
          `northstar-ledger-${today}.csv`,
          exportLedgerCsv(filteredLedger, accountName, { includeNotes }),
        );
        files += 1;
      }
      if (includeInvestments && filteredInvestments.length) {
        ...
        downloadCsv(
          `northstar-investments-${today}.csv`,
          exportInvestmentCsv(filteredInvestments, assetFor),
        );
        files += 1;
      }
  ...
  const fieldToggles = [
    {
      key: "transfers",
      on: includeTransfers,
      set: setIncludeTransfers,
      label: t("settings.transfers"),
    },
    {
      key: "investments",
      on: includeInvestments,
      set: setIncludeInvestments,
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <Card className="p-5">
        <div
          className="text-xs"
          style={{ marginBottom: 10, color: "var(--ns-fg-muted)", fontWeight: 500 }}
        >
          {t("settings.format")}
        </div>
          ...
          {formatCards.map((f) => (
            <button
              ...
              style={{
                ...
                background: format === f.id ? "var(--ns-accent-soft)" : "transparent",
                color: "var(--ns-fg)",
              }}
            >
      ...
      {jsonMode ? (
        <Card className="p-5">
          <div className="flex items-start gap-2 text-sm muted">
            ...
            <span>
              JSON 為<strong style={{ color: "var(--ns-fg)" }}>整份資料庫</strong>
              的完整備份（含帳戶、交易、投資、設定、匯率），不套用下方的時間 / 帳戶 /
              欄位篩選。可於「{t("settings.general")}」分頁用此檔還原。
            </span>
          </div>
        </Card>
      ) : (
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {(
                ...
                <Button
                  ...
                  onClick={() => setRange(id)}
                  style={{
                    borderColor: range === id ? "var(--ns-accent)" : "var(--ns-border)",
                    background: range === id ? "var(--ns-accent-soft)" : "transparent",
                  }}
                >
                  {label}
                </Button>
          ...
          <Card className="p-5">
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {ACCOUNT_SCOPES.map((s) => {
                const count = s.types
                  ? accountRows.filter((a) => s.types!.includes(a.type)).length
                  : accountRows.length;
                return (
                  <Button
                    key={s.id}
                    ...
                    style={{
                      borderColor: scope === s.id ? "var(--ns-accent)" : "var(--ns-border)",
                      background: scope === s.id ? "var(--ns-accent-soft)" : "transparent",
                    }}
                  >
                    {s.label}
          ...
          <Card className="p-5">
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {fieldToggles.map((f) => (
                ...
                >
                  {f.on ? <CheckCircle size={14} weight="bold" /> : <Plus size={14} />}
                  {f.label}
                </button>
              ))}
            </div>
