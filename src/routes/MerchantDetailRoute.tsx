...
export function MerchantDetailRoute() {
  ...
  const { ledger, accounts, settings, dailyFxRates, isInitialLoading, isError, error, refetchAll } =
    useFinanceData();
  ...
  const categorySetting = appSettings?.categories.find(
    (category) => category.name === categoryHit.value,
  );
  const categoryColor = categorySetting?.color ?? "var(--ns-accent)";
  const filterOptions = [
    ...new Set(rows.map((row) => row.subcategory || row.category).filter(Boolean)),
  ];
  const visibleRows = periodRows.filter(
    (row) =>
      subcategoryFilter === "all" ||
      row.subcategory === subcategoryFilter ||
      row.category === subcategoryFilter,
  );
  ...
  const relatedMerchants = useMemo(() => {
    ...
    for (const row of ledgerRows) {
      if (
        ...
        row.entryType !== "expense" ||
        row.settlementStatus !== "settled" ||
        row.category !== categoryHit.value ||
        !row.merchant ||
    ...
    <div className="ns-detail-page">
      <div className="ns-detail-header">
          ...
          <div className="ns-detail-title-row">
            ...
            <div className="min-w-0">
              ...
              <div className="ns-pill-row">
                {categoryHit.value ? (
                  <span className="ns-filter-pill" data-active>
                    {categoryHit.value}
                  </span>
                ) : null}
                {subcategoryHit.value ? (
                  <span className="ns-filter-pill">{subcategoryHit.value}</span>
                ) : null}
              </div>
            </div>
          </div>
        ...
        <div className="ns-detail-actions">
          ...
          <Button
            ...
            onClick={() =>
              downloadCsv(
                `northstar-merchant-${merchantName}.csv`,
      ...
      <div className="ns-detail-grid">
        <div className="ns-detail-main">
          ...
          <Panel
            eyebrow="消費模式"
            title={peakWeekday ? `星期${peakWeekday.name} 消費最高` : "星期分佈"}
          >
          ...
          <Panel eyebrow="交易紀錄" title={`${visibleRows.length} 筆交易紀錄`}>
            <div className="ns-pill-row ns-transaction-filter-row">
              <button
                ...
                data-active={subcategoryFilter === "all" || undefined}
              ...
              {filterOptions.map((filter) => (
                <button
                  ...
                  type="button"
                  className="ns-filter-pill"
                  data-active={subcategoryFilter === filter || undefined}
        ...
        <div className="ns-detail-side">
            ...
            {categoryHit.value ? (
              <div className="ns-rule-hint">
                ...
                <div>
                  ...
                  <span className="mono">
                    {merchantName.toUpperCase()} → {categoryHit.value}
                    {subcategoryHit.value ? ` › ${subcategoryHit.value}` : ""}
                  </span>
                  <span>
                    下次用快速記帳（⌘N）輸入「{merchantName}」時，會自動套用這個分類，不用每次手選。
                  </span>
                </div>
              </div>
            ) : (
              <EmptyPanel
                icon={<Tag size={22} />}
                text="交易筆數還不足，無法自動推導分類；多記幾筆後，快速記帳就會自動帶入。"
              />
            )}
          </Panel>
            ...
            <InfoList
              rows={[
                ...
                [
                  "分類命中率",
                  categoryHit.total
                    ? `${((categoryHit.count / categoryHit.total) * 100).toFixed(0)}%`
                    : "—",
                ],
              ]}
            />
          ...
          <Panel
            eyebrow={`同類商家${categoryHit.value ? ` · ${categoryHit.value}` : ""}`}
            title="同類商家"
          >
        ...
        <table className="ns-detail-table">
          ...
          <tfoot>
            <tr>
              ...
              <td className={`num text-right ${total < 0 ? "pos" : "neg"}`}>
                {total < 0 ? "+" : "−"}
                {formatMoney(Math.abs(total), primaryCurrency)}
              </td>
            </tr>
          </tfoot>
        </table>
...
function Initials({ name, color, large }: { name: string; color?: string; large?: boolean }) {
  return (
    <span
      className={large ? "ns-initials ns-initials-lg" : "ns-initials"}
