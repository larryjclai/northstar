...
import {
  buildQuoteLookup,
  findQuoteForTicker,
  isTaiwanListedTicker,
} from "../domain/marketSymbols";
...
function perPeriodCash(
  rule: Pick<RecurringInvestment, "mode" | "amount" | "quantity" | "price" | "fee">,
): number {
  const base =
    rule.mode === "fixedShares" ? (rule.quantity || 0) * (rule.price || 0) : rule.amount || 0;
...
export function RecurringInvestmentsTab() {
  ...
  const updateRule = useRepositoryMutation(
    (repo, input: RecurringInvestmentDraft & { id: string }) =>
      repo.updateRecurringInvestment(input.id, input),
    ["recurringInvestments"],
  );
  ...
  function openCreate() {
    ...
    setDraft({
      ...
      currency: investmentAccounts[0]?.currency ?? "TWD",
      nextRunDate: today,
    });
    setSheetOpen(true);
  }
  ...
  async function submit() {
    ...
    if (draft.mode === "fixedAmount" && !(draft.amount > 0)) {
      toast.error("請輸入每期金額");
      return;
    }
    if (draft.mode === "fixedShares" && !(draft.quantity > 0)) {
      toast.error("請輸入每期股數");
      return;
    }
    try {
    ...
    <div>
      <div className="flex items-center justify-between mb-4 gap-3" style={{ flexWrap: "wrap" }}>
        <div className="muted text-body">
          {rules.filter((r) => r.isActive).length} 個進行中 · 每月約投入{" "}
          <span className="num" style={{ color: "var(--ns-fg)" }}>
            NT${formatNumber(Math.round(monthlyTotal))}
          </span>
        </div>
      ...
      {investmentAccounts.length === 0 ? (
        <Card className="p-8 text-center">
          ...
          <div className="muted text-body">
            請先在「帳戶」新增券商（投資）帳戶，才能設定定期定額並扣交割款。
          </div>
        </Card>
      ) : rules.length === 0 ? (
        <Card className="p-8 text-center">
          ...
          <div className="muted text-body mb-4">
            設定定期定額或定期定股，到期時會在總覽與投資頁提醒你補交割款。
          </div>
          <Button onClick={openCreate}>
            <Plus size={14} weight="bold" />
            建立第一個計畫
        ...
        <Card className="p-0">
          {rules.map((rule, i) => {
            ...
            return (
              ...
              >
                <div
                  className="text-caption flex items-center shrink-0 font-semibold"
                  style={{
                    width: 38,
                    height: 38,
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {recurringInvestmentModeLabels[rule.mode]}
                    </Badge>
                    {due ? (
                      <Badge
                        variant="outline"
                  ...
                  <div className="muted text-xs flex items-center gap-1.5" style={{ marginTop: 2 }}>
                    <CalendarBlank size={14} /> {freqLabel(rule)} · {accountName(rule.accountId)} ·
                    下次 {rule.nextRunDate.slice(5)}
                  </div>
                </div>
                <div className="text-right">
                  <div className="num text-sm font-medium">
                    {rule.mode === "fixedShares"
                      ? `${formatQuantity(rule.quantity)} 股`
                      : `NT$${formatNumber(rule.amount)}`}
                  </div>
                  <div className="muted mono text-caption">
                    交割約 NT${formatNumber(Math.round(perPeriodCash(rule)))}
                  </div>
                </div>
                <div className="flex gap-1">
                  <Button
                    ...
                    size="icon-sm"
                    title="記錄本期投入"
                    onClick={() => setPostConfirm(rule)}
                    disabled={postRule.isPending}
                  >
                    <Check size={14} />
                  </Button>
                  <Button
...
function PostConfirmDialog({
  ...
  return (
    <ModalShell
      variant="center"
      title={`確認記錄本期投入 · ${rule.name || rule.ticker}`}
      onClose={onClose}
        ...
        <Card className="w-full p-0">
          ...
          >
            <h2 className="text-base font-semibold" style={{ margin: 0 }}>
              確認記錄本期投入 · {rule.name || rule.ticker}
            </h2>
          ...
          <div className="py-4 px-5 flex flex-col gap-3.5">
            <div>
              <div className="text-body font-medium">
                參考價 NT${formatNumber(rule.price, { maximumFractionDigits: 2 })}
              </div>
              <div className="muted text-xs" style={{ marginTop: 2 }}>
                {cashNote(rule.price)}
              </div>
              {wholeShareNote(rule.price) ? (
                <div className="muted text-xs" style={{ marginTop: 2 }}>
                  {wholeShareNote(rule.price)}
                </div>
              ) : null}
            ...
            <div>
              <div className="text-body font-medium">
                最新報價{" "}
                {latest !== null
                  ? `NT$${formatNumber(latest, { maximumFractionDigits: 2 })}`
                  : "無報價資料"}
              </div>
              {latest !== null ? (
                <>
              ...
              {latest !== null && differs ? (
                <>
                  ...
                  <Button
                    variant="outline"
                    className="w-full justify-center"
                    onClick={() => onPostLatest(rule, latest)}
                    disabled={pending}
                    loading={pending}
                  >
      ...
      {(dismiss) => (
        <>
          <div
            ...
            style={{ padding: "18px 22px", borderBottom: "1px solid var(--ns-border)" }}
          >
            <h2
              className="text-lg m-0 font-semibold"
              style={{ fontFamily: "var(--ns-font-display)" }}
            >
              {editing ? "編輯定期定額" : "新增定期定額"}
            </h2>
            <ModalCloseButton onClick={dismiss} />
          </div>
          ...
          <div className="flex flex-1 flex-col gap-4" style={{ overflow: "auto", padding: 22 }}>
              ...
              <TickerSearchField
                ...
                onSelect={(r) =>
                  setDraft({
                    ...
                    currency: r.currency || draft.currency,
                  })
                }
              />
            </Field>
                ...
                <AppSelect
                  ...
                  onChange={(id) => {
                    ...
                    setDraft({
                      ...draft,
                      accountId: id === "all" ? "" : id,
                      currency: a?.currency ?? draft.currency,
                    });
                  }}
                  options={[
                    { value: "all", label: "選擇帳戶" },
            ...
            <div className="gap-3.5" style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
              <Field label="頻率">
                <AppSelect
                  ...
                  onChange={(frequency) =>
                    setDraft({ ...draft, frequency: frequency as RecurringFrequency })
                  }
                  options={(Object.keys(recurringFrequencyLabels) as RecurringFrequency[]).map(
                    (f) => ({ value: f, label: recurringFrequencyLabels[f] }),
                  )}
                  style={{ width: "100%", height: 40 }}
                />
              </Field>
            ...
            <Field label="下次執行日">
              <input
                className="ns-input"
                type="date"
                value={draft.nextRunDate}
                onChange={(e) => setDraft({ ...draft, nextRunDate: e.target.value })}
              />
            </Field>
            ...
            <Card
              className="text-xs flex items-center gap-2 py-2.5 px-3.5"
              style={{ flexDirection: "row", color: "var(--ns-fg-muted)" }}
            >
              <ArrowsClockwise size={14} /> 每期預估交割款約{" "}
              <span className="num" style={{ color: "var(--ns-fg)" }}>
                NT${formatNumber(Math.round(cash))}
              </span>
              ，記錄時會自動扣款。
            </Card>
          </div>
          ...
          <div
            ...
            style={{ padding: "14px 22px", borderTop: "1px solid var(--ns-border)" }}
          >
            <Button
              variant="ghost"
