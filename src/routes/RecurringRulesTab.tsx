import { ArrowsClockwise, CalendarBlank, Check, Plus, Trash } from "@phosphor-icons/react";
...
export function RecurringRulesTab() {
  ...
  const updateRecurring = useRepositoryMutation(
    (repo, input: RecurringDraft & { id: string }) =>
      repo.updateRecurringTransaction(input.id, input),
    ["recurring"],
  );
  ...
  const allFilterOptions: { key: FreqFilter; label: string }[] = [
    ...
    {
      key: "monthly",
      label: `每月 (${activeRules.filter((r) => r.frequency === "monthly").length})`,
    },
    {
      key: "yearly",
      label: `每年 (${activeRules.filter((r) => r.frequency === "yearly").length})`,
    },
    {
      key: "weekly",
      label: `每週 (${activeRules.filter((r) => r.frequency === "weekly").length})`,
    },
  ...
  const filterOptions = allFilterOptions.filter(
    (o) =>
      o.key === "all" ||
      o.key === "paused" ||
      parseInt(o.label.match(/\d+/)?.[0] ?? "0") > 0 ||
      filter === o.key,
  );
  ...
  return (
    <div className="flex flex-col" style={{ gap: "var(--ns-gap-card)" }}>
      ...
      >
        <KpiCard
          label="月收入（預估）"
          value={`NT$${formatNumber(monthlyIncome)}`}
          color="var(--ns-pos)"
        />
        <KpiCard
          label="月支出（預估）"
          value={`NT$${formatNumber(monthlyExpense)}`}
          color="var(--ns-neg)"
        />
        <KpiCard
          label="月淨現金流"
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        ...
        <div
          ...
          style={{ padding: "16px 20px", borderBottom: "1px solid var(--ns-border)" }}
        >
          ...
          <Button
            className="text-xs"
            style={{ padding: "5px 12px", minHeight: "auto" }}
            onClick={openCreate}
          >
            <Plus weight="bold" />
            新增規則
            ...
            filtered.map((rule) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  >
                    {rule.entryType === "income" ? "+" : "−"}NT$
                    {formatNumber(Math.abs(monthlyEquivalent(rule)))}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-2">
                  <span className="muted truncate text-xs">
                    {rule.category}
                    {rule.subcategory ? ` / ${rule.subcategory}` : ""} · {freqLabel(rule)} ·{" "}
        ...
        <div className="hidden sm:contents">
          ...
          {filtered.length > 0 && (
            <div
              ...
              style={{
                display: "grid",
                gridTemplateColumns: "1fr 90px 110px 110px 120px 80px",
                columnGap: 16,
                padding: "8px 20px",
                borderBottom: "1px solid var(--ns-border)",
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((rule) => (
              ...
              >
                ...
                <span
                  className="text-xs"
                  style={{ color: "var(--ns-fg-dim)", alignSelf: "center" }}
                >
                  {freqLabel(rule)}
                </span>
                <span
...
function KpiCard({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <Card style={{ padding: "14px 16px" }}>
      ...
      <div className="num text-lg font-semibold" style={{ color }}>
        {value}
      </div>
    </Card>
  );
...
function RuleEditSheet({
  ...
  const signedAmount =
    form.entryType === "expense" ? -Math.abs(form.amount) : Math.abs(form.amount);
        ...
        {(dismiss) => (
          <>
            ...
            <div
              ...
              style={{ padding: "18px 24px", borderBottom: "1px solid var(--ns-border)" }}
            >
              <ArrowsClockwise size={16} style={{ color: "var(--ns-accent)" }} />
              <span className="text-[15px] font-semibold">
                {isCreating ? "新增週期規則" : "編輯週期規則"}
              </span>
              <div className="flex-1" />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <RuleField label="週期">
                  <AppSelect
                    ...
                    onChange={(frequency) =>
                      setForm({ ...form, frequency: frequency as RecurringDraft["frequency"] })
                    }
                    options={[
                ...
                <AppSelect
                  ...
                  onChange={(id) => {
                    ...
                    setForm({
                      ...form,
                      accountId: id === "all" ? "" : id,
                      currency: acct?.currency ?? form.currency,
                    });
                  }}
                  options={[
                ...
                >
                  <div
                    style={{
                      ...
                      height: 18,
                      borderRadius: 9,
                      background: form.isActive ? "var(--ns-pos)" : "var(--ns-fg-dim)",
                      position: "relative",
              ...
              {!isCreating && (
                <div
                  ...
                  style={{
                    ...
                    border: "1px solid var(--ns-neg-soft)",
                  }}
                >
                  <div className="text-xs font-semibold mb-2" style={{ color: "var(--ns-neg)" }}>
                    刪除規則
                  </div>
                  {confirmDelete ? (
                    <div className="flex gap-2 items-center">
                      <span className="text-xs flex-1" style={{ color: "var(--ns-fg-muted)" }}>
                        確定刪除？此操作無法復原。
                      </span>
                      <Button
                        variant="destructive"
...
function RuleField({
  ...
  return (
    <div>
      ...
      >
        {label}
        {required && <span style={{ color: "var(--ns-neg)", marginLeft: 3 }}>*</span>}
      </label>
      {children}
    </div>
  );
