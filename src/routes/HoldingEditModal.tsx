...
export function HoldingEditModal({
  ...
  const updateHolding = useRepositoryMutation(
    (repository, input: PortfolioAssetDraft & { id: string }) =>
      repository.updateManualHolding(input.id, input),
    ["assets"],
  );
  const updateClassification = useRepositoryMutation(
    (
      ...
      repository.updateAssetClassification(input.id, {
        ...
        <>
          ...
          <div className="max-h-[70vh] overflow-y-auto px-5 pb-5 pt-4">
            <HoldingForm
              ...
              onSubmit={submitEdit}
              submitLabel={
                updateHolding.isPending || updateClassification.isPending
                  ? "儲存中…"
                ...
                {hasYahoo && (editingAsset.holdingSource !== "manual" || priceMode === "auto") ? (
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {yahooPrices
                        .slice((pricePage - 1) * PRICE_PAGE_SIZE, pricePage * PRICE_PAGE_SIZE)
                        .map((p) => (
                          <div
                            key={p.date}
                    ...
                    {Math.ceil(yahooPrices.length / PRICE_PAGE_SIZE) > 1 ? (
                      <div className="mt-3 flex items-center justify-center gap-3 text-xs">
                        ...
                        <span style={{ color: "var(--ns-muted)" }}>
                          {pricePage} / {Math.ceil(yahooPrices.length / PRICE_PAGE_SIZE)}
                        </span>
                        <ActionButton
                          variant="secondary"
                          size="sm"
                          disabled={pricePage >= Math.ceil(yahooPrices.length / PRICE_PAGE_SIZE)}
                          onClick={() =>
                            setPricePage((p) =>
                              Math.min(Math.ceil(yahooPrices.length / PRICE_PAGE_SIZE), p + 1),
                            )
                          }
                        >
                          下一頁
                  ...
                  <>
                    {snapshots.length === 0 ? (
                      <p className="mb-3 text-xs" style={{ color: "var(--ns-muted)" }}>
                        尚無快照，新增第一筆後就能在績效圖中看到趨勢。
                      </p>
                    ) : (
                        ...
                        {snapshots.map((snap) => (
                          ...
                          >
                            <div>
                              ...
                              {snap.note ? (
                                <span className="ml-2 text-xs" style={{ color: "var(--ns-muted)" }}>
                                  {snap.note}
                                </span>
                              ) : null}
                            </div>
                    ...
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_1fr_auto]">
                      <Field label="日期">
                        <DatePicker
                          value={snapshotDate}
                          onChange={(val) => setSnapshotDate(val)}
                      ...
                      <Field label="淨值 / 價格">
                        <TextInput
                          type="number"
                          value={snapshotPrice}
                          onChange={(e) => setSnapshotPrice(Number(e.target.value))}
                        />
                      </Field>
                    ...
                    <div className="mt-2">
                      <Field label="備註（選填）">
                        <TextInput
                          ...
                          placeholder="基金公告淨值 2026-05-26"
                        />
                      </Field>
                    </div>
                    {snapshotMessage ? (
                      <div className="mt-2">
                        <StatusText>{snapshotMessage}</StatusText>
                      </div>
                    ) : null}
                  </>
                ) : null}
