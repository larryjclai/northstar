...
export function InvestmentImportWizard({ open, onClose, accounts, onImport }: Props) {
  ...
  async function runImport() {
    ...
    try {
      ...
      toast.success(
        `已匯入 ${investmentCount} 筆交易${cashCount ? `、${cashCount} 筆入出金` : ""}`,
      );
      onClose();
    } catch (error) {
      ...
      {(dismiss) => (
        <>
          ...
          <div
            ...
            style={{ padding: "16px 22px", borderBottom: "1px solid var(--ns-border)" }}
          >
            <div className="text-lg font-semibold" style={{ fontFamily: "var(--ns-font-display)" }}>
              匯入證券交易
            </div>
          ...
          >
            {steps.map((s, i) => {
              ...
              return (
                ...
                >
                  <div
                    className="text-caption flex items-center justify-center"
                    style={{
                      width: 22,
                      height: 22,
                      borderRadius: 99,
                      border: `1.5px solid ${active ? "var(--ns-accent)" : "var(--ns-border)"}`,
                      background: done ? "var(--ns-accent)" : "transparent",
                      color: done ? "var(--ns-accent-fg)" : "var(--ns-fg)",
                    }}
                  >
                    {done ? <CheckCircle size={13} weight="bold" /> : i + 1}
                  </div>
                  <span className="text-body" style={{ fontWeight: active ? 600 : 400 }}>
                    {s.label}
                  </span>
              ...
              <div style={{ display: "grid", gap: 18 }}>
                <div>
                  ...
                  {investAccounts.length === 0 ? (
                    <div
                      className="ns-surface p-3 text-sm"
                      style={{ border: "1px solid var(--ns-neg)" }}
                    >
                  ...
                  {hasRowAccount ? (
                    <div className="muted text-xs mt-1.5">
                      這份 CSV 含有帳戶欄位，預覽時會優先用每列的 accountName /
                      accountId；上方帳戶只作為找不到對應時的預設值。
                    </div>
                  ) : null}
                </div>
                ...
                <div>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      border: `1.5px dashed ${fileName ? "var(--ns-accent)" : "var(--ns-border)"}`,
                      background: "transparent",
                      color: "var(--ns-fg)",
                    }}
                  >
                    {fileName ? (
                      <FileCsv size={26} style={{ color: "var(--ns-accent)" }} />
                    ) : (
                    ...
                    <div className="muted text-xs">
                      {fileName
                        ? `${rows.length} 列 · 分隔符 ${delimiter === "\t" ? "Tab" : delimiter}`
                        : "僅支援 CSV"}
                    </div>
                  </button>
                </div>
                ...
                {headers.length > 0 && (
                  <div>
                    ...
                    <PreviewTable
                      headers={previewHeaders(headers, mapping)}
              ...
              <div className="grid gap-5">
                <div>
                  ...
                  >
                      ...
                      return (
                        <div key={field}>
                          <label
                            ...
                            style={{ color: unset ? "var(--ns-neg)" : "var(--ns-fg-muted)" }}
                          >
                            {FIELD_LABELS[field]}
                            {required && <span style={{ color: "var(--ns-neg)" }}> *</span>}
                          </label>
                          <AppSelect
                            ...
                            style={{
                              width: "100%",
                              height: 40,
                              borderColor: unset ? "var(--ns-neg)" : undefined,
                            }}
                          />
                        </div>
                      );
                    ...
                    <div>
                      <label className="text-xs block mb-1" style={{ color: "var(--ns-fg-muted)" }}>
                        日期格式
                      </label>
                      <AppSelect
                        value={mapping.dateFormat}
                        onChange={(dateFormat) =>
                          setMapping((p) => ({ ...p, dateFormat: dateFormat as DateFormat }))
                        }
                        options={(Object.keys(DATE_FORMAT_LABELS) as DateFormat[]).map((f) => ({
                          value: f,
                        ...
                        return (
                          ...
                          >
                            ...
                            <AppSelect
                              value={cur ?? ""}
                              onChange={(action) =>
                                setActivity(value, action as ImportActivity | "ignore")
                              }
                              options={[
                                ...(pending ? [{ value: "", label: "請選擇" }] : []),
                                ...IMPORT_ACTION_VALUES.map((action) => ({
                                  value: action,
                                  label: IMPORT_ACTION_LABELS[action],
                                })),
                              ...
                              style={{
                                width: "100%",
                                height: 40,
                                borderColor: pending ? "var(--ns-neg)" : undefined,
                              }}
                            />
                          </div>
                        );
                ...
                <div className="pt-4" style={{ borderTop: "1px solid var(--ns-border)" }}>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <input
                      ...
                      placeholder="範本名稱（例：Firstrade）"
                      value={templateName}
                      onChange={(e) => setTemplateName(e.target.value)}
                    />
                    {templateId ? (
                      <Button variant="outline" onClick={() => saveTemplate(false)}>
                        <FloppyDisk size={14} />
                        更新範本
                      </Button>
                    ) : null}
                    <Button variant="outline" onClick={() => saveTemplate(true)}>
                      <FloppyDisk size={14} />
                      儲存為新範本
                    </Button>
                    {templateId ? (
                      <Button variant="destructive-outline" onClick={removeTemplate}>
                        <Trash size={14} />
                        刪除
                      </Button>
                    ) : null}
              ...
              <div className="grid gap-4">
                <div className="flex gap-3 flex-wrap">
                  ...
                  <StatPill
                    ...
                    tone={preview.invalid.length ? "warn" : "muted"}
                  />
                  <StatPill
                    label="目標帳戶"
                    value={hasRowAccount ? "依 CSV 每列" : account ? `${account.name}` : "—"}
                    tone="muted"
                  />
                </div>
                ...
                {preview.invalid.length > 0 && (
                  ...
                  >
                    <div className="text-sm font-medium mb-2" style={{ color: "var(--ns-neg)" }}>
                      未匯入的列
                    </div>
                    {preview.invalid.map((item) => (
                      <div key={item.row} className="text-xs" style={{ marginBottom: 3 }}>
                ...
                {importError ? (
                  <div
                    className="ns-surface p-3"
                    style={{ border: "1px solid var(--ns-neg)", borderRadius: "var(--ns-r-md)" }}
                  >
                    <div className="text-sm font-medium mb-1" style={{ color: "var(--ns-neg)" }}>
                      匯入失敗
                    </div>
                    <div className="text-xs" style={{ whiteSpace: "pre-wrap", lineHeight: 1.6 }}>
                      {importError}
                    </div>
                  </div>
                ) : null}
                ...
                {preview.valid.length > 0 && (
                  <div>
                    <div className="muted text-sm mb-2">
                      前 {Math.min(8, preview.valid.length)} 筆預覽
                    </div>
                      ...
                      >
                        ...
                        <tbody>
                          {preview.valid.slice(0, 8).map((item) => (
                            <ImportPreviewRow key={item.row} item={item} />
                          ))}
                        </tbody>
                      </table>
                    </div>
                ...
                <div className="muted text-caption flex items-start gap-1.5">
                  ...
                  <span>
                    買入交易需要該投資帳戶有足夠現金交割；若帳戶餘額不足，匯入時該筆會被拒絕並顯示原因。
                  </span>
                </div>
              </div>
            )}
          ...
          <div
            ...
            style={{ padding: "16px 22px", borderTop: "1px solid var(--ns-border)" }}
          >
            <Button
              variant="ghost"
              onClick={() => {
            ...
            {step === "upload" && (
              <Button disabled={!canMap} onClick={() => setStep("mapping")}>
                欄位對應
                <ArrowRight size={14} />
              </Button>
            )}
            ...
            {step === "review" && (
              ...
              >
                ...
                {importing ? "匯入中…" : `確認匯入 ${preview?.valid.length ?? 0} 筆`}
              </Button>
            )}
          </div>
        </>
...
function buildImportPlan(
  items: Array<{ row: number; value: InvestmentImportValue }>,
): InvestmentActivityImportPlan {
...
function PreviewTable({
  ...
  return (
    <div>
      {totalColumns > headers.length ? (
        <div className="muted text-caption mb-1.5">
          只顯示 {headers.length} 個關鍵欄位，另有 {totalColumns - headers.length}{" "}
          欄會在匯入時保留解析。
        </div>
      ) : null}
      <div
      ...
      >
        <table
          ...
          style={{
            width: "100%",
            minWidth: Math.min(760, headers.length * 120),
  ...
  if (value.kind === "cash") {
    ...
    return (
      <tr>
        ...
        <td style={{ padding: "6px 10px" }} className="num">
          {sign}
          {value.draft.currency} {Math.abs(value.draft.amount)}
        </td>
        <td style={{ padding: "6px 10px" }} className="muted">
          —
        </td>
...
function StatPill({
  ...
  const color =
    tone === "ok" ? "var(--ns-accent)" : tone === "warn" ? "var(--ns-neg)" : "var(--ns-fg-muted)";
  return (
    <div
      style={{
        padding: "10px 14px",
    ...
    >
      <div className="text-xs ns-field-label" style={{ marginBottom: 2 }}>
        {label}
      </div>
