...
export const ICON_GROUPS: { label: string; names: string[] }[] = [
  ...
  {
    label: "生活",
    names: ["Heart", "FirstAid", "Pill", "Barbell", "GraduationCap", "BookOpen", "PawPrint"],
  },
  {
    label: "娛樂",
    names: [
  ...
  {
    label: "帳單",
    names: ["Lightning", "Drop", "Flame", "Phone", "DeviceMobile", "Laptop", "WifiHigh"],
  },
  {
    label: "其他",
    names: [
...
export const ICON_KEYWORDS: Record<string, string> = {
  ...
  Money: "鈔票 現金",
  Receipt: "收據 發票 帳單",
  CurrencyCircleDollar: "美元 貨幣 錢",
  CurrencyBtc: "比特幣 加密貨幣",
  HandCoins: "薪水 收入 借貸",
  ChartLineUp: "投資 股票 成長",
  ChartPieSlice: "資產配置 圓餅",
  TrendUp: "上漲 趨勢",
  Briefcase: "工作 公事包",
  Scales: "貸款 天秤 平衡",
  House: "房子 房產 居家",
  ...
  Pill: "藥 保健",
  Barbell: "健身 運動 重訓",
  GraduationCap: "教育 學費 學習",
  BookOpen: "書 閱讀 學習",
  PawPrint: "寵物 貓狗",
  FilmSlate: "電影 影音",
  GameController: "遊戲 娛樂",
  MusicNotes: "音樂 訂閱",
  Camera: "相機 攝影",
  Headphones: "耳機 音樂",
  SoccerBall: "運動 球賽",
...
export function Glyph({
  ...
  if (text) {
    return (
      <span
        className={className}
        style={{ fontSize: name ? size : Math.round(size * 0.7), color, lineHeight: 1, ...style }}
      >
        {text}
      </span>
    );
