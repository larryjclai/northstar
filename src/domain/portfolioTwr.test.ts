...
function dailySeries(ticker: string, from: string, n: number, priceFn: (i: number) => number): DailyPrice[] {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    out.push({ ticker, date, close: priceFn(i), currency: "USD", source: "test", updatedAt: `${date}T00:00:00.000Z` });
  }
  return out;
}
...
const position: AnalyticsPosition = { assetId: "a", ticker: "AAA", quantity: 20, currency: "USD", isManual: false };
...
describe("buildPortfolioTwr", () => {
  it("is cash-flow neutral: a mid-window top-up does not create a return", () => {
    ...
    const twr = buildPortfolioTwr({ positions: [position], records, dailyPrices: prices, toPrimary: identity, start: "2026-01-01", end: "2026-02-04" });
  ...
  it("equals price return when there are no in-window flows", () => {
    ...
    const twr = buildPortfolioTwr({ positions: [{ ...position, quantity: 10 }], records, dailyPrices: prices, toPrimary: identity, start: "2026-01-01", end: "2026-02-04" });
  ...
  it("gates on too-few observations and discloses unpriced tickers", () => {
    ...
    const records = [record({ assetId: "a", date: "2025-12-01", action: "buy", price: 100, quantity: 10 })];
    const twr = buildPortfolioTwr({ positions: [{ ...position, quantity: 10 }], records, dailyPrices: prices, toPrimary: identity, start: "2026-01-01", end: "2026-01-05" });
    expect(twr.twrPct).toBeNull(); // < MIN_ANALYTICS_DAYS

    const noHistory = buildPortfolioTwr({ positions: [{ ...position, ticker: "ZZZ" }], records: [], dailyPrices: prices, toPrimary: identity, start: "2026-01-01", end: "2026-02-04" });
