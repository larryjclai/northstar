...
describe("buildDividendAnalysis", () => {
  ...
  it("returns null yield when market value is unknown", () => {
    const records = [
      record({ assetId: "a", date: "2026-03-01", action: "cashDividend", price: 500, quantity: 0 }),
    ];
    const r = buildDividendAnalysis({
      records,
      assetMeta: meta,
  ...
  it("is empty when there are no dividends", () => {
    const records = [
      record({ assetId: "a", date: "2026-03-01", action: "buy", price: 100, quantity: 5 }),
    ];
    const r = buildDividendAnalysis({
      records,
      assetMeta: meta,
