...
export function TransactionDetailPanel({
  ...
  async function submitRefund() {
    ...
    if (isNaN(parsed) || parsed <= 0) {
      setRefundError("請輸入有效金額");
      return;
    }
    if (parsed > maxAbs) {
      setRefundError(`退款金額不能超過 ${maxAbs}`);
      return;
    }
  ...
  const linkedRule =
    row.recurringRuleId && recurringRows
        ...
        <>
          ...
          <div className="flex-1" style={{ overflow: "auto", padding: "28px 24px" }}>
            ...
            <div className="text-center" style={{ marginBottom: 32 }}>
              ...
              >
                {meta.sign}
                {row.currency === "TWD" ? "NT$" : row.currency + " "}
              ...
              <div className="flex justify-center mt-2 gap-1.5" style={{ flexWrap: "wrap" }}>
                ...
                {!isSettled && settlementLabel && (
                  <span
                    ...
                    style={{
                      padding: "3px 10px",
                      borderRadius: 99,
                      background: "var(--ns-chart-3)" + "18",
                      color: "var(--ns-chart-3)",
                    }}
                  >
                    {settlementLabel}
                  </span>
                ...
                {row.recurringRuleId ? (
                  ...
                  >
                    ...
                    週期交易
                    {linkedRule ? ` · ${recurringFrequencyLabels[linkedRule.frequency]}` : ""}
                  </span>
                ) : (
                  <span
                    className="text-caption font-medium"
            ...
            <div className="flex flex-col gap-0">
              ...
              {isReimbursement && arApRoles ? (
                <>
                  <DetailField
                    icon={<Wallet size={15} />}
                    label={arApRoles.payLabel}
                    value={arApRoles.payValue}
                  />
                  <DetailField
                    icon={<Wallet size={15} />}
                    label={arApRoles.receiveLabel}
                    value={arApRoles.receiveValue}
                  />
                </>
              ) : (
                <DetailField
                  ...
                  label="帳戶"
                  value={
                    row.accountId ? accountName(row.accountId) : isSettled ? "—" : "結清時指定"
                  }
                />
              )}
            ...
            {onSettle &&
            !isSettled &&
            (row.settlementStatus === "receivable" || row.settlementStatus === "payable") ? (
              ...
              <div className="mt-6 pt-5" style={{ borderTop: "1px solid var(--ns-border)" }}>
                {!refundOpen ? (
                  <Button
                    ...
                    onClick={() => {
                      setRefundOpen(true);
                      setRefundAmount(String(Math.abs(row.amount)));
                      setRefundError("");
                    }}
                  >
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <label className="text-xs muted">
                      ...
                      <input
                        className="ns-input mt-1"
                        type="date"
                        value={refundDate || row.date.slice(0, 10)}
          ...
          {/* Footer Actions */}
          <div
            className="flex py-4 px-6 gap-2.5"
...
function DetailField({
  ...
  return (
    <div
      className="items-center py-3.5 px-0 gap-2"
      style={{
        display: "grid",
        gridTemplateColumns: "28px 72px 1fr",
